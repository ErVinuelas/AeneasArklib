...
| `GOLD_P = 2^64 − 2^32 + 1` arithmetic: `>> 32`, `& 0xFFFFFFFF`, `wrapping_mul`, `u128` sum with a conditional subtract (2026-09-19) | all pure or ordinary monadic `U64`/`U128` ops, zero axioms | candidate T27's whole arithmetic layer. Nothing new in the constructs — the row is here because the *proof* consequence is unusual: `AuxArith`'s `Magic p m` requires `p < 2^32`, so a 64-bit auxiliary prime cannot instantiate it and needs its own `AuxGold`. The compensation is that `2^64 ≡ 2^32 − 1 (mod GOLD_P)` exactly, so `gold_reduce_spec` and `gold_mul_spec` carry **no precondition at all** |

| `v[i] = v[i] + x` in **one expression**, `v : Vec<u64>` (2026-09-19) | **aeneas aborts**: `Unimplemented binary operation` | measured on candidate T33's `ring::short_pass_off`. Read the slot into a `let` first -- `let cur = v[i]; let nv = cur + x; v[i] = nv;` -- and it extracts clean. It works unchanged for `Vec<Ext4>` (`AuxShift.shift_accum` does exactly `acc[m] = acc[m] + …`), so the trigger is the **scalar** element type: rustc lowers the read-modify-write on a place of scalar type to an operation charon does not model |
