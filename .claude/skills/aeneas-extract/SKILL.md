...
| shared read at an **absolute offset** `pfwd[base + k]` beside an in-place write `out[k] = …` in one loop body, behind a helper boundary (2026-09-21) | `base + k` is a plain monadic `Usize` add, then `Vec.index`; loop state stays the 2-tuple `(out1, k1)`; zero axioms | card T34 part 1B. Probed because a borrowed read beside a mutated accumulator is what tripped `filter_loop_useless_inputs_outputs` once -- inside its own `fn` it does not. The same shape written into the **caller's** loop is still the failure case |
| a `&Rq` operand read coefficientwise while a second `Vec<u64>` is written in place, one loop (2026-09-21) | loop state is the **3-tuple** `(a, w, t)` -- the borrowed operand rides along, as in `add_loop`/`sub_loop` | card T34 part 1A, `ring::load_twisted_into`. A spec for it carries `r.1 = a` as an invariant conjunct; `subst` then discharges it definitionally, so the `refine` has one slot fewer than the conjunct count suggests |
| **runtime** shift amount: `(x >> (4 * e)) & 15` with `e : usize` (2026-09-17) | `let i ← 4#usize * e; let i1 ← x >>> i; ok (i1 &&& 15#u64)` -- monadic `>>>`, pure `&&&`, zero axioms | the shift's side condition is `i.val < 64`, so the caller owes `e < 16`; `Std.U64.ShiftRight_spec` is `@[step]` and gives `z.val = x.val >>> i.val`. Probed, not reasoned |
| `pub const T: [u32; 8]` indexed by a runtime `usize` (2026-09-17) | `Array Std.U32 8#usize` + `Array.index_usize` | same as the inherited 4-element row, at a second width and element type |
| reading a borrowed `&Vec<u64>` inside a loop that also writes a `Vec` accumulator (2026-09-17) | **aeneas aborts**: `Aeneas.Errors.CFailure` in `PureMicroPassesLoops.filter_loop_useless_inputs_outputs` | measured on `ring::dot_prep_chunk_mod_p`. Hoisting the read into a local `Vec` *before* the accumulate loop does **not** fix it; the loop nesting itself is the problem |
| the same, with each inner loop factored into its own top-level `fn` (2026-09-17) | extracts clean, zero axioms | the fix: `ring::slice_out` and `ring::mac_into` as separate functions flatten the nesting the analysis chokes on. Also the `lean-to-rust` preference anyway -- smaller functions, smaller loop specs |
...
| `pub struct RawRq32(Vec<u32>)` — a newtype over a **non-`Fp`** `Vec`, and `RawVec32(Vec<RawRq32>)` **nested** over it (2026-09-18) | `@[reducible] def ring.RawRq32 := alloc.vec.Vec Std.U32` and `@[reducible] def linalg.RawVec32 := alloc.vec.Vec ring.RawRq32`; all four loops 2-tuples, zero axioms | candidate T29's carrier. Both get the same `@[reducible]` alias treatment as `Rq`/`PolyVec`, *including* the nested one, so a statement about a `RawVec32` is one about the `Vec` and `Check.lean` § 2b's container-newtype assertion extends to it unchanged. The element default in a `getD` is then `alloc.vec.Vec.new ring.RawRq32`, not a scalar |
| `x as u32` **narrowing** on a value the caller knows is reduced (2026-09-18) | `lift (UScalar.cast .U32 i1)` — **fallible** | discharged by `UScalar.cast_inBounds_spec .U32 w (h : w.val ≤ UScalar.max .U32)`. The side condition is exactly `Field.Red` when the source is an `Fp` (`q = 2^32 − 99 < 2^32`), so on a `Wf` operand it costs **no new precondition** — which is what makes a `u32` carrier for reduced coefficients admissible under "totality over preconditions" |
| `w as u64` **widening** (2026-09-18) — *corrects the inherited row* | `lift (UScalar.cast .U64 i1)`, **not** `ok (…)` | the AeneasCompPoly row reads "`UScalar.cast`" without saying which; measured here, a widening is `lift` too, i.e. still monadic. It is nonetheless *always* dischargeable: `UScalar.cast_inBounds_spec .U64 _ (by scalar_tac)`, because `scalar_tac` knows the source type's own bound. Five places in this tree already spell the `have hcast : lift (UScalar.cast .U64 x) ⦃ y => y.val = x.val ⦄` idiom |
| a `Vec`-valued `let` bound from an extracted function, then **pushed** into an accumulator inside a counter loop (2026-09-18) | loop state stays the 2-tuple `(out, i)`; the pushed element is an ordinary `Vec.push` | measured on `RawVec32::expand`/`compact`, whose bodies are `out.push(self.0[i].expand())`. Relevant because the element is itself a container: the state does **not** grow to carry it |
| **`match` on a custom two-variant enum**, one variant carrying a `Vec` — the T1b ceiling probe (2026-09-18) | `@[discriminant isize] inductive Shape where | Dense : alloc.vec.Vec Std.U64 → Shape | Structured : Std.U64 → Shape`, and a **native Lean `match`** in every consumer; a `structure` HOLDING the enum keeps a real `m : Shape` field and its method matches on `self.m` directly; a counter loop calling that method keeps the **2-tuple** state; 0 axioms, 0 `Shared` prefixes | the construct the ceiling table listed as unprobed, and it **passes**. Probed for candidate T1b, where `RlinStatement` must hold *either* the dense `M` or a block-structured description of it: `RlinStatement::new`'s signature is pinned by four `define_cases!` bodies, so the dense constructor has to survive, and an enum is what lets the representation change underneath it without duplicating the 16 functions that take `&RlinStatement`. Proof shape: the `match` is discharged once inside `entry`'s spec, so consumers never case-split — they `step` through `entry_spec`. `probe-enum/` in this session's scratchpad |
| **`overflowing_add` / `overflowing_sub`** (2026-09-19) | **NOT SUPPORTED** — not modelled | measured while building T27's Goldilocks lane. The replacement matters: `checked_add` + `match` on the `Option`, which IS in this table, costs **7.995 ns per butterfly against 2.787** for the same arithmetic, because the `Option` match defeats the carry-flag path. The admissible *and* fast form is a `u128` intermediate — `(a as u128) + (b as u128)`, compare, subtract, cast back — which is already in the table above |
| a **`u128` modular add/sub** over a modulus near `2^64` (2026-09-19) | `lift (UScalar.cast .U128 …)`, checked `+`/`-`, a `u128` comparison, `lift (UScalar.cast .U64 …)` back | the shape a >32-bit auxiliary prime forces, since `Magic`'s `p < 2^32` is what keeps a product of two residues in a `u64`. Extracts clean, zero axioms |
| indexing a **tuple field inside a loop** — `fwb.0[k]` where `fwb : (Vec α × Vec α)` (2026-09-19) | **aeneas ERROR: "Not an open binder or an ignored pattern"**, and the whole extraction fails | the crash is in a `PureMicroPassesLoops` pass and the message does not name the construct, so it is worth recognising. Binding the parts out first and passing a slice to a helper — exactly what `ring::dot_prep_chunk_mod_p` already does with `mac_into` — extracts fine. That existing shape is not a style choice; it is the one that works. Passing `&fwb.0` to a function is fine; *indexing* it in a loop is not |

| indexing a **tuple field** inside a loop: `fwb.0[k]` where `fwb : (Vec<u64>, Vec<u64>)` (2026-09-19) | **aeneas aborts**: `Not an open binder or an ignored pattern` | measured on candidate T27's `ring::dot_prepared_digits_gold` at `src/ring.rs:999`. The tuple comes from `gold_forward`, which returns `(out, scratch)`. Fix: bind the parts out *above* the loop (`let (fwd, _) = gold_forward(…);`) and pass a slice to a separate helper — the shape `ring::dot_prep_chunk_mod_p` already uses, and the same flattening the 2026-09-17 `CFailure` row asks for |
| `u64::overflowing_add` / `overflowing_sub` as an **arithmetic** idiom (2026-09-19) — *scope note on the 2026-09-16 row* | modelled (that row stands), but **not** a substitute for an overflow-checked add you intend to prove about | measured while shaping T27's Goldilocks butterfly. Of the three admissible forms, `u128` intermediates cost **2.787 ns/butterfly**, `checked_add` + `match` on the `Option` costs **7.995 ns — 3× worse**, because the `Option` match defeats the carry-flag path. Prefer the `u128` intermediate; it is in the inherited table and it is the fast one |
| `GOLD_P = 2^64 − 2^32 + 1` arithmetic: `>> 32`, `& 0xFFFFFFFF`, `wrapping_mul`, `u128` sum with a conditional subtract (2026-09-19) | all pure or ordinary monadic `U64`/`U128` ops, zero axioms | candidate T27's whole arithmetic layer. Nothing new in the constructs — the row is here because the *proof* consequence is unusual: `NttArith`'s `Magic p m` requires `p < 2^32`, so a 64-bit auxiliary prime cannot instantiate it and needs its own `GoldArith`. The compensation is that `2^64 ≡ 2^32 − 1 (mod GOLD_P)` exactly, so `gold_reduce_spec` and `gold_mul_spec` carry **no precondition at all** |

| `v[i] = v[i] + x` in **one expression**, `v : Vec<u64>` (2026-09-19) | **aeneas aborts**: `Unimplemented binary operation` | measured on candidate T33's `ring::short_pass_off`. Read the slot into a `let` first -- `let cur = v[i]; let nv = cur + x; v[i] = nv;` -- and it extracts clean. It works unchanged for `Vec<Ext4>` (`SumcheckShift.shift_accum` does exactly `acc[m] = acc[m] + …`), so the trigger is the **scalar** element type: rustc lowers the read-modify-write on a place of scalar type to an operation charon does not model |
