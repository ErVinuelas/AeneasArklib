---
name: route-r3
description: The hybrid route (the default) — optimize a targeted ArkLib definition on the Lean side with bench-steered iteration, then pay the proof debt per accepted champion; run when asked to execute route R3, the default optimization route, or an end-to-end optimize-and-verify pass on one target
---
...
**Human invocation:** start with `/route-r3` alone. Ask: **“Which ArkLib
...
1. `arklib-analyze` — target definition in → optimization brief out.
