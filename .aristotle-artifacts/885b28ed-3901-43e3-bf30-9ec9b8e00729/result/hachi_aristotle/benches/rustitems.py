...
Two jobs depend on knowing where each item in `hachi/src/*.rs` starts and stops:

* **genesis fidelity** — proving that a frozen baseline function is byte-for-byte
  what `hachi/src` contained at the commit its annotation names, and
* **bench coverage** — pairing every ``Mirrors `ArkLib.X` `` docstring with a bench
