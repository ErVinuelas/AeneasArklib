...
/// `honestZ` [`honest_z`] mirrors, composed with `gadgetDecompose`).
///
...
// ---------------------------------------------------------------------------
//
// Its ArkLib home is `RingSwitch/Rlin.lean`, but it lives here rather than in
// `ringswitch.rs` because `lib.rs`'s layering forbids the other placement:
// `stack`/`unstack` are maps on `QuadEvalResponse`, and `ringswitch` is
// declared *below* `quadeval`. The adapter consumes QuadEval's output and
// produces a `ringswitch::RlinStatement`, so it can only sit on the QuadEval
// side of that boundary. Every dimension travels as an argument, which the
// freeze decision requires (NOTES.md § "Decision: the `R^lin` adapter's
// genesis holds the reshaped form").
