...
/// (Stage 6 candidates T1a and T45a).
///
/// A dispatch on the matrix's representation (card T45a). An assembled
/// [`RlinMat::Dense`] matrix carries no block structure to read, so its arm is
/// the per-entry walk this function *was* until T45a, verbatim -- kept inline
/// rather than routed through [`band_high`], which measured +17-22% on the
/// dense rows (a second caller changes what the compiler inlines). A
/// [`RlinMat::Lazy`] one gets
/// [`c_row_sum_high_lazy`], which walks row `i`'s bands block by block and
/// contracts gadget-shaped groups before the product. The two return the same
/// words on every well-formed lazy statement; which one runs is decided by the
/// representation and by run-time checks, never by a hypothesis, so
/// `c_row_sum_high_spec` keeps its statement (rule 4, totality over
/// preconditions).
///
...
fn c_row_sum_high(s: &RlinStatement, z: &PolyVec, i: usize) -> Vec<Fp> {
    match s.m() {
        ...
        RlinMat::Lazy(b) => c_row_sum_high_lazy(b, z, i),
    }
}
