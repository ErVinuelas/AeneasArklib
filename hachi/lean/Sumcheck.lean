...
/-! ### Card T38: the coefficients directly

`round_poly_alpha` used to evaluate at three nodes and interpolate. The
summand is a product of two *affine* folds, so it is a quadratic in `T` whose
coefficients are available without evaluating anything, and card T38 builds
them. The statement is unchanged; what left the proof is `interpolate_spec`,
which is why this is shorter than what it replaces. -/

/-- The even-indexed entry of a table, as an element of `F`. -/
def evenF (v : alloc.vec.Vec cpoly.field.Ext4) (y : ℕ) : F :=
  toExt (v.val.getD (2 * y) cpoly.field.Ext4.ZERO)

/-- The odd-indexed entry of a table, as an element of `F`. -/
def oddF (v : alloc.vec.Vec cpoly.field.Ext4) (y : ℕ) : F :=
  toExt (v.val.getD (2 * y + 1) cpoly.field.Ext4.ZERO)

/-- The three coefficients one pair contributes, over abstract readers, so
that the plain and the split forms share them: the split one differs only in
how `Ã`'s two entries are read. -/
def qC0 (W0 A0 : ℕ → F) (y : ℕ) : F := W0 y * A0 y
def qC2 (W0 W1 A0 A1 : ℕ → F) (y : ℕ) : F := (W1 y - W0 y) * (A1 y - A0 y)
def qC1 (W0 W1 A0 A1 : ℕ → F) (y : ℕ) : F :=
  W1 y * A1 y - qC0 W0 A0 y - qC2 W0 W1 A0 A1 y

/-- **The identity the card rests on**: the product of two affine folds, in the
coefficients the loop accumulates. -/
theorem pair_quadratic (W0 W1 A0 A1 : ℕ → F) (y : ℕ) (x : F) :
    ((1 - x) * W0 y + x * W1 y) * ((1 - x) * A0 y + x * A1 y)
      = qC0 W0 A0 y + x * qC1 W0 W1 A0 A1 y + x ^ 2 * qC2 W0 W1 A0 A1 y := by
  simp only [qC0, qC1, qC2]
  ring

/-- The `y` loop of `round_poly_alpha`: three accumulators carrying the partial
sums of the three per-pair coefficients. -/
theorem round_poly_alpha_loop_spec {k : ℕ} (w a_tab : alloc.vec.Vec cpoly.field.Ext4)
    (half : Std.Usize) (c0 c1 c2 : cpoly.field.Ext4) (y : Std.Usize)
    (hwlen : w.val.length = 2 ^ (k + 1)) (hwred : VecReduced w)
    (halen : a_tab.val.length = 2 ^ (k + 1)) (hared : VecReduced a_tab)
    (hhalfv : half.val = 2 ^ k) (hyb : y.val ≤ 2 ^ k)
    (hR0 : Reduced c0) (hR1 : Reduced c1) (hR2 : Reduced c2)
    (hv0 : toExt c0 = ∑ t ∈ Finset.range y.val, qC0 (evenF w) (evenF a_tab) t)
    (hv1 : toExt c1 = ∑ t ∈ Finset.range y.val, qC1 (evenF w) (oddF w) (evenF a_tab) (oddF a_tab) t)
    (hv2 : toExt c2 = ∑ t ∈ Finset.range y.val, qC2 (evenF w) (oddF w) (evenF a_tab) (oddF a_tab) t) :
    sumcheck.round_poly_alpha_loop w a_tab half c0 c1 c2 y
      ⦃ z => Reduced z.1 ∧ Reduced z.2.1 ∧ Reduced z.2.2
        ∧ toExt z.1 = ∑ t ∈ Finset.range (2 ^ k), qC0 (evenF w) (evenF a_tab) t
        ∧ toExt z.2.1 = ∑ t ∈ Finset.range (2 ^ k), qC1 (evenF w) (oddF w) (evenF a_tab) (oddF a_tab) t
        ∧ toExt z.2.2 = ∑ t ∈ Finset.range (2 ^ k), qC2 (evenF w) (oddF w) (evenF a_tab) (oddF a_tab) t ⦄ := by
  have hmax := usize_max_ge
  rw [sumcheck.round_poly_alpha_loop]
  apply loop.spec_decr_nat (fun s => 2 ^ k - s.2.2.2.val)
    (fun s => s.2.2.2.val ≤ 2 ^ k ∧ Reduced s.1 ∧ Reduced s.2.1 ∧ Reduced s.2.2.1
      ∧ toExt s.1 = ∑ t ∈ Finset.range s.2.2.2.val, qC0 (evenF w) (evenF a_tab) t
      ∧ toExt s.2.1 = ∑ t ∈ Finset.range s.2.2.2.val, qC1 (evenF w) (oddF w) (evenF a_tab) (oddF a_tab) t
      ∧ toExt s.2.2.1 = ∑ t ∈ Finset.range s.2.2.2.val, qC2 (evenF w) (oddF w) (evenF a_tab) (oddF a_tab) t)
  · rintro ⟨d0, d1, d2, u⟩ ⟨hu, hD0, hD1, hD2, he0, he1, he2⟩
    dsimp only at hu hD0 hD1 hD2 he0 he1 he2
    simp only [sumcheck.round_poly_alpha_loop.body]
    by_cases hlt : u < half
    · rw [if_pos hlt]
      have hult : u.val < 2 ^ k := by
        have : u.val < half.val := by scalar_tac
        omega
      have hwb : 2 * u.val + 1 < w.val.length := by rw [hwlen, pow_succ]; omega
      have hab : 2 * u.val + 1 < a_tab.val.length := by rw [halen, pow_succ]; omega
      step as ⟨i, hi⟩
      have hib : i.val < w.val.length := by rw [hi]; omega
      have hiba : i.val < a_tab.val.length := by rw [hi]; omega
      step as ⟨e, he⟩
      ...
      step as ⟨e1, he1'⟩
      have hRe1 : Reduced e1 := he1' ▸ hared _ (List.getElem_mem hiba)
      step as ⟨p0, hRp0, hp0⟩
      step as ⟨i1, hi1⟩
      have hi1b : i1.val < w.val.length := by rw [hi1, hi]; omega
      step as ⟨e2, he2'⟩
      ...
      have hi2b : i2.val < a_tab.val.length := by rw [hi2, hi]; omega
      step as ⟨e3, he3⟩
      have hRe3 : Reduced e3 := he3 ▸ hared _ (List.getElem_mem hi2b)
      step as ⟨p1, hRp1, hp1⟩
      step as ⟨i3, hi3⟩
      have hi3b : i3.val < w.val.length := by rw [hi3, hi]; omega
      step as ⟨e4, he4⟩
      have hRe4 : Reduced e4 := he4 ▸ hwred _ (List.getElem_mem hi3b)
      step as ⟨e5, he5⟩
      have hRe5 : Reduced e5 := he5 ▸ hwred _ (List.getElem_mem hib)
      step as ⟨e6, hRe6, he6⟩
      step as ⟨i4, hi4⟩
      have hi4b : i4.val < a_tab.val.length := by rw [hi4, hi]; omega
      step as ⟨e7, he7⟩
      have hRe7 : Reduced e7 := he7 ▸ hared _ (List.getElem_mem hi4b)
      step as ⟨e8, he8⟩
      have hRe8 : Reduced e8 := he8 ▸ hared _ (List.getElem_mem hiba)
      step as ⟨e9, hRe9, he9⟩
      step as ⟨p2, hRp2, hp2⟩
      step as ⟨c01, hRc01, hc01⟩
      step as ⟨e10, hRe10, he10⟩
      step as ⟨e11, hRe11, he11⟩
      step as ⟨c11, hRc11, hc11⟩
      step as ⟨c21, hRc21, hc21⟩
      step as ⟨u1, hu1⟩
      -- every read, named back to `evenF`/`oddF`
      have gEw : toExt e = evenF w u.val := by
        rw [he, evenF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hib, hi]
      have gEw5 : toExt e5 = evenF w u.val := by
        rw [he5, evenF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hib, hi]
      have gOw2 : toExt e2 = oddF w u.val := by
        rw [he2', oddF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hi1b, hi1, hi]
      have gOw4 : toExt e4 = oddF w u.val := by
        rw [he4, oddF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hi3b, hi3, hi]
      have gEa : toExt e1 = evenF a_tab u.val := by
        rw [he1', evenF, ← List.getD_eq_getElem a_tab.val cpoly.field.Ext4.ZERO hiba, hi]
      have gEa8 : toExt e8 = evenF a_tab u.val := by
        rw [he8, evenF, ← List.getD_eq_getElem a_tab.val cpoly.field.Ext4.ZERO hiba, hi]
      have gOa3 : toExt e3 = oddF a_tab u.val := by
        rw [he3, oddF, ← List.getD_eq_getElem a_tab.val cpoly.field.Ext4.ZERO hi2b, hi2, hi]
      have gOa7 : toExt e7 = oddF a_tab u.val := by
        rw [he7, oddF, ← List.getD_eq_getElem a_tab.val cpoly.field.Ext4.ZERO hi4b, hi4, hi]
      refine ⟨by scalar_tac, hRc01, hRc11, hRc21, ?_, ?_, ?_, by scalar_tac⟩
      · rw [hc01, he0, hu1, Finset.sum_range_succ, hp0, gEw, gEa, qC0]
      · rw [hc11, he1, hu1, Finset.sum_range_succ, he11, he10, hp1, hp2, he6, he9,
          gOw2, gOa3, hp0, gEw, gEa, gOw4, gEw5, gOa7, gEa8, qC1, qC0, qC2]
      · rw [hc21, he2, hu1, Finset.sum_range_succ, hp2, he6, he9,
          gOw4, gEw5, gOa7, gEa8, qC2]
    · rw [if_neg hlt, WP.spec_ok]
      dsimp only
      have hueq : u.val = 2 ^ k := by
        have : half.val ≤ u.val := by scalar_tac
        omega
      exact ⟨hD0, hD1, hD2, by rw [he0, hueq], by rw [he1, hueq], by rw [he2, hueq]⟩
  · exact ⟨hyb, hR0, hR1, hR2, hv0, hv1, hv2⟩

/-- `round_poly_alpha` builds the quadratic's three coefficients directly, and
they evaluate to `linSumAlpha` everywhere (card T38).

The statement is the one the interpolating version had. -/
theorem round_poly_alpha_spec {k : ℕ} (w a_tab : alloc.vec.Vec cpoly.field.Ext4)
  ...
  step with round_poly_alpha_loop_spec (k := k) w a_tab half
    cpoly.field.Ext4.ZERO cpoly.field.Ext4.ZERO cpoly.field.Ext4.ZERO 0#usize
    hwlen hwred halen hared hhalfv (by simp) hRZ hRZ hRZ
    (by simp [toExt_ZERO]) (by simp [toExt_ZERO]) (by simp [toExt_ZERO])
    as ⟨c0, c1, c2, hR0, hR1, hR2, hc0, hc1, hc2⟩
  simp only [alloc.vec.Vec.with_capacity]
  have hcap : ∀ (v : alloc.vec.Vec cpoly.field.Ext4), v.val.length < Usize.max →
      True := fun _ _ => trivial
  step as ⟨v1, hv1⟩
  step as ⟨v2, hv2⟩
  step as ⟨v3, hv3⟩
  rw [cpoly.univariate.UnivariatePoly.from_coeffs, WP.spec_ok]
  have hv1l : v1.val.length = 1 := by rw [hv1]; simp
  have hv2l : v2.val.length = 2 := by rw [hv2, List.length_append, hv1l]; simp
  have hv3l : v3.val.length = 3 := by rw [hv3, List.length_append, hv2l]; simp
  refine ⟨hv3l, ?_, fun x => ?_⟩
  · intro z hz
    rw [hv3] at hz
    ...
    · rw [hv2] at h
      ...
      · rw [hv1] at h2
        ...
        · simp at h3
        · rw [List.mem_singleton.mp h3]; exact hR0
      · rw [List.mem_singleton.mp h2]; exact hR1
    · rw [List.mem_singleton.mp h]; exact hR2
  -- the three coefficients, read back
  ...
  have hg2 : v3.val.getD 2 cpoly.field.Ext4.ZERO = c2 := by
    rw [hv3, ← hv2l]
    exact getD_append_eq _ _ _
  rw [toUni_eval, toRaw_eval_eq_sum v3 x 3 (by rw [hv3l]),
    Finset.sum_range_succ, Finset.sum_range_succ, Finset.sum_range_succ,
    Finset.sum_range_zero, zero_add]
  have hco : ∀ t : ℕ, t < 3 → (toRaw v3).coeff t
      = toExt (v3.val.getD t cpoly.field.Ext4.ZERO) := by
    intro t ht
    rw [toRaw_coeff_of_lt v3 (by rw [hv3l]; exact ht),
      ← List.getD_eq_getElem (l := v3.val) (d := cpoly.field.Ext4.ZERO) (by rw [hv3l]; exact ht)]
  rw [hco 0 (by norm_num), hco 1 (by norm_num), hco 2 (by norm_num),
    hg0, hg1, hg2, hc0, hc1, hc2, linSumAlpha_eq_sum_range]
  simp only [Finset.sum_mul]
  rw [← Finset.sum_add_distrib, ← Finset.sum_add_distrib]
  refine Finset.sum_congr rfl (fun t _ => ?_)
  -- `pair_quadratic`, inlined: the identity is the same one, and unfolding
  -- here spares a rewrite that would have to match `evenF`/`oddF`
  -- syntactically against the `getD`s `linSumAlpha_eq_sum_range` produces.
...
/-! ### Round 1's zero side off the round-0 base table (card T46b)

`round_poly_zero_fold1 w_fp a0 eq` is, by its first translation, the
composition `round_poly_zero (eval_mle_layer_base w_fp a0) eq`; its conclusion
is `round_poly_zero_spec`'s at that table, stated through
`eval_mle_layer_base_spec`'s value -- the fold of the embedded base table at
`a0`. From `2^21` pairs up the code first tries the bucketed branch
(`round_poly_zero_fold1_bucketed`): it buckets the round-1 pairs by the quad
type of their four round-0 digits (`bucket_quads_base`,
`lean/SumcheckBucket.lean`), folds the representative table
(`quad_type_table_base`) at the same `a0`, and runs `round_poly_zero` on the
`2^19` types; `bucket_quads_sum_eq` regroups that sum back to the pairs. -/

/-- The composition itself: `eval_mle_layer_base` then `round_poly_zero`. -/
theorem round_poly_zero_fold1_plain_spec {k : ℕ} (w_fp : alloc.vec.Vec cpoly.field.Fp)
    (a0 : cpoly.field.Ext4) (eq : alloc.vec.Vec cpoly.field.Ext4)
    (hw : WfEvalsFp (k + 1 + 1) w_fp) (heq : WfEvals k eq) (ha0 : Reduced a0) :
    (do
      let w1 ← sumcheck.eval_mle_layer_base w_fp a0
      sumcheck.round_poly_zero w1 eq)
      ⦃ out => out.val.length = 33 ∧ VecReduced out ∧
        ∀ x : F, CPolynomial.eval x (toUni out) =
          rangeSumZero (fold (phiF ∘ tableFnFp (m := k + 1 + 1) w_fp) (toExt a0))
            (tableFn (m := k) eq) x ⦄ := by
  step with eval_mle_layer_base_spec (k := k + 1) w_fp a0 hw ha0 as ⟨w1, hW1, hw1v⟩
  apply spec_mono (round_poly_zero_spec (k := k) w1 eq hW1 heq)
  rintro out ⟨h1, h2, h3⟩
  refine ⟨h1, h2, fun x => ?_⟩
  rw [h3 x, show tableFn (m := k + 1) w1
      = fold (phiF ∘ tableFnFp (m := k + 1 + 1) w_fp) (toExt a0) from funext hw1v]

/-- `round_poly_zero_fold1_bucketed`: `None`, or the composition's value, from
the `2^19` quad-type buckets. -/
theorem round_poly_zero_fold1_bucketed_spec {k : ℕ} (w_fp : alloc.vec.Vec cpoly.field.Fp)
    (a0 : cpoly.field.Ext4) (eq : alloc.vec.Vec cpoly.field.Ext4)
    (hw : WfEvalsFp (k + 1 + 1) w_fp) (heq : WfEvals k eq) (ha0 : Reduced a0) :
    sumcheck.round_poly_zero_fold1_bucketed w_fp a0 eq
      ⦃ o => match o with
        | none => True
        | some out => out.val.length = 33 ∧ VecReduced out ∧
            ∀ x : F, CPolynomial.eval x (toUni out) =
              rangeSumZero (fold (phiF ∘ tableFnFp (m := k + 1 + 1) w_fp) (toExt a0))
                (tableFn (m := k) eq) x ⦄ := by
  have hwl : w_fp.val.length = 4 * eq.val.length := by
    rw [hw.1, heq.1, pow_succ, pow_succ]; ring
  rw [sumcheck.round_poly_zero_fold1_bucketed]
  step with HachiEquiv.SumcheckBucket.bucket_quads_base_spec w_fp eq hwl hw.2 heq.2
    as ⟨o, ho⟩
  rcases o with _ | b
  · exact trivial
  · obtain ⟨hbl, hbr, hdec, hbv⟩ := ho
    step with HachiEquiv.SumcheckBucket.quad_type_table_base_spec as ⟨v, hvl, hvr, hvv⟩
    step with eval_mle_layer_base_spec (k := 19 + 1) v a0 ⟨by rw [hvl]; norm_num, hvr⟩ ha0
      as ⟨reps, hWr, hrv⟩
    step with round_poly_zero_spec (k := 19) reps b hWr ⟨by rw [hbl]; norm_num, hbr⟩
      as ⟨up, h1, h2, h3⟩
    refine ⟨h1, h2, fun x => ?_⟩
    rw [h3 x, show tableFn (m := 19 + 1) reps
        = fold (phiF ∘ tableFnFp (m := 19 + 1 + 1) v) (toExt a0) from funext hrv]
    unfold rangeSumZero
    exact HachiEquiv.SumcheckBucket.bucket_quads_sum_eq w_fp v eq b
      (InnerOuter.rangeProduct 16) (toExt a0) x heq.1 hwl hw.2 hvv hdec hbv

/-- **`round_poly_zero_fold1` is round 1's range summand off the round-0 base
table** (card T46b): `33` reduced coefficients whose polynomial evaluates to
`rangeSumZero` at the fold of the embedded table `w_fp` at `a0` -- the table
`eval_mle_layer_base w_fp a0` denotes (`eval_mle_layer_base_spec`), which is
what `round_poly_zero_spec` would say on it. Both branches land there: the
composition directly, the bucketed one through `bucket_quads_sum_eq`. -/
theorem round_poly_zero_fold1_spec {k : ℕ} (w_fp : alloc.vec.Vec cpoly.field.Fp)
    (a0 : cpoly.field.Ext4) (eq : alloc.vec.Vec cpoly.field.Ext4)
    (hw : WfEvalsFp (k + 1 + 1) w_fp) (heq : WfEvals k eq) (ha0 : Reduced a0) :
    sumcheck.round_poly_zero_fold1 w_fp a0 eq
      ⦃ out => out.val.length = 33 ∧ VecReduced out ∧
        ∀ x : F, CPolynomial.eval x (toUni out) =
          rangeSumZero (fold (phiF ∘ tableFnFp (m := k + 1 + 1) w_fp) (toExt a0))
            (tableFn (m := k) eq) x ⦄ := by
  rw [sumcheck.round_poly_zero_fold1]
  split
  · step with round_poly_zero_fold1_bucketed_spec w_fp a0 eq hw heq ha0 as ⟨o, ho⟩
    rcases o with _ | p
    · exact round_poly_zero_fold1_plain_spec w_fp a0 eq hw heq ha0
    · exact ho
  · exact round_poly_zero_fold1_plain_spec w_fp a0 eq hw heq ha0
...
/-- `linSumAlphaSplitFp_eq_sum_range` at an arbitrary point `x : F` rather than
an embedded node `phiF T`: the two `w̃` entries are embedded one at a time and
the fold itself is taken in `F`. The shape card T41b1's direct-coefficient
`round_poly_alpha_base_split` is compared against, whose conclusion quantifies
over every `x : F` and which evaluates at no node. -/
theorem linSumAlphaSplitFp_eq_sum_range_ext {j k κ : ℕ} (hjk : j + k = κ + 1)
    (w : alloc.vec.Vec cpoly.field.Fp) (low high : alloc.vec.Vec cpoly.field.Ext4)
    (x : F) :
    linSumAlpha (phiF ∘ tableFnFp (m := κ + 1) w)
        (reidx hjk (tensorTable (tableFn (m := k) low) (tableFn (m := j) high))) x
      = ∑ y ∈ Finset.range (2 ^ κ),
          ((1 - x) * phiF (coeffK w (2 * y)) + x * phiF (coeffK w (2 * y + 1))) *
            ((1 - x) * tensorRead low high (2 ^ k) (2 * y) +
              x * tensorRead low high (2 ^ k) (2 * y + 1)) := by
  rw [linSumAlpha, ← Fin.sum_univ_eq_sum_range (fun y : ℕ =>
    ((1 - x) * phiF (coeffK w (2 * y)) + x * phiF (coeffK w (2 * y + 1))) *
      ((1 - x) * tensorRead low high (2 ^ k) (2 * y) +
        x * tensorRead low high (2 ^ k) (2 * y + 1))) (2 ^ κ)]
  refine Finset.sum_congr rfl (fun y _ => ?_)
  simp only [fold, tableFn_apply, reidx, tensorTable, tensorRead, lo, hi,
    Function.comp_apply, tableFnFp]
...
theorem round_poly_alpha_split_pairs_loop_spec {j k κ : ℕ} (hjk : j + k = κ + 1)
    ...
    (hw : WfEvals (κ + 1) w) (hlow : WfEvals k low) (hhigh : WfEvals j high)
    (hhalfv : half.val = 2 ^ κ) (hl : l.val = 2 ^ k) (hyb : y.val ≤ 2 ^ κ)
    (hR0 : Reduced c0) (hR1 : Reduced c1) (hR2 : Reduced c2)
    (hv0 : toExt c0 = ∑ t ∈ Finset.range y.val,
      qC0 (evenF w) (fun u => tensorRead low high (2 ^ k) (2 * u)) t)
    (hv1 : toExt c1 = ∑ t ∈ Finset.range y.val,
      qC1 (evenF w) (oddF w) (fun u => tensorRead low high (2 ^ k) (2 * u))
    ...
    sumcheck.round_poly_alpha_split_pairs_loop w low high half l c0 c1 c2 y
      ⦃ z => Reduced z.1 ∧ Reduced z.2.1 ∧ Reduced z.2.2
        ...
        ∧ toExt z.2.2 = ∑ t ∈ Finset.range (2 ^ κ),
            qC2 (evenF w) (oddF w) (fun u => tensorRead low high (2 ^ k) (2 * u))
              (fun u => tensorRead low high (2 ^ k) (2 * u + 1)) t ⦄ := by
  obtain ⟨hwlen, hwred⟩ := hw
  obtain ⟨hlowlen, hlowred⟩ := hlow
  obtain ⟨hhighlen, hhighred⟩ := hhigh
  have hwmax : w.val.length ≤ Usize.max := w.property
  ...
  apply loop.spec_decr_nat (fun s => 2 ^ κ - s.2.2.2.val)
    (fun s => s.2.2.2.val ≤ 2 ^ κ ∧ Reduced s.1 ∧ Reduced s.2.1 ∧ Reduced s.2.2.1
      ∧ toExt s.1 = ∑ t ∈ Finset.range s.2.2.2.val,
          qC0 (evenF w) (fun u => tensorRead low high (2 ^ k) (2 * u)) t
      ∧ toExt s.2.1 = ∑ t ∈ Finset.range s.2.2.2.val,
          qC1 (evenF w) (oddF w) (fun u => tensorRead low high (2 ^ k) (2 * u))
            (fun u => tensorRead low high (2 ^ k) (2 * u + 1)) t
      ∧ toExt s.2.2.1 = ∑ t ∈ Finset.range s.2.2.2.val,
          qC2 (evenF w) (oddF w) (fun u => tensorRead low high (2 ^ k) (2 * u))
            (fun u => tensorRead low high (2 ^ k) (2 * u + 1)) t)
  · rintro ⟨d0, d1, d2, u⟩ ⟨hu, hD0, hD1, hD2, he0, he1, he2⟩
    dsimp only at hu hD0 hD1 hD2 he0 he1 he2
    ...
    · rw [if_pos hlt]
      ...
      have hib : i.val < w.val.length := by rw [hi]; omega
      step as ⟨i1, hi1⟩
      have hi1b : i1.val < low.val.length := by
        rw [hlowlen, hi1, hl]; exact Nat.mod_lt _ (by rw [← hl]; exact hlpos)
      step as ⟨e, he⟩
      have hRe : Reduced e := he ▸ hlowred _ (List.getElem_mem hi1b)
      step as ⟨i2, hi2⟩
      have hi2b : i2.val < high.val.length := by
        rw [hhighlen, hi2, hi, hl]
        refine Nat.div_lt_of_lt_mul ?_
        rw [htwo]; omega
      step as ⟨e1, he1'⟩
      have hRe1 : Reduced e1 := he1' ▸ hhighred _ (List.getElem_mem hi2b)
      step as ⟨a0, hRa0, ha0⟩
      step as ⟨i3, hi3⟩
      step as ⟨i4, hi4⟩
      have hi4b : i4.val < low.val.length := by
        rw [hlowlen, hi4, hl]; exact Nat.mod_lt _ (by rw [← hl]; exact hlpos)
      step as ⟨e2, he2'⟩
      have hRe2 : Reduced e2 := he2' ▸ hlowred _ (List.getElem_mem hi4b)
      step as ⟨i5, hi5⟩
      step as ⟨i6, hi6⟩
      have hi6b : i6.val < high.val.length := by
        rw [hhighlen, hi6, hi5, hi, hl]
        refine Nat.div_lt_of_lt_mul ?_
        rw [htwo]; omega
      step as ⟨e3, he3⟩
      have hRe3 : Reduced e3 := he3 ▸ hhighred _ (List.getElem_mem hi6b)
      step as ⟨a1, hRa1, ha1⟩
      step as ⟨e4, he4⟩
      have hRe4 : Reduced e4 := he4 ▸ hwred _ (List.getElem_mem hib)
      step as ⟨p0, hRp0, hp0⟩
      step as ⟨i7, hi7⟩
      have hi7b : i7.val < w.val.length := by rw [hi7, hi]; omega
      step as ⟨e5, he5⟩
      have hRe5 : Reduced e5 := he5 ▸ hwred _ (List.getElem_mem hi7b)
      step as ⟨p1, hRp1, hp1⟩
      step as ⟨i8, hi8⟩
      have hi8b : i8.val < w.val.length := by rw [hi8, hi]; omega
      step as ⟨e6, he6⟩
      have hRe6 : Reduced e6 := he6 ▸ hwred _ (List.getElem_mem hi8b)
      step as ⟨e7, he7⟩
      have hRe7 : Reduced e7 := he7 ▸ hwred _ (List.getElem_mem hib)
      step as ⟨e8, hRe8, he8⟩
      step as ⟨e9, hRe9, he9⟩
      step as ⟨p2, hRp2, hp2⟩
      step as ⟨c01, hRc01, hc01⟩
      ...
      step as ⟨c21, hRc21, hc21⟩
      step as ⟨u1, hu1⟩
      have gW0 : toExt e4 = evenF w u.val := by
        rw [he4, evenF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hib, hi]
      have gW0' : toExt e7 = evenF w u.val := by
        rw [he7, evenF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hib, hi]
      have gW1 : toExt e5 = oddF w u.val := by
        rw [he5, oddF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hi7b, hi7, hi]
      have gW1' : toExt e6 = oddF w u.val := by
        rw [he6, oddF, ← List.getD_eq_getElem w.val cpoly.field.Ext4.ZERO hi8b, hi8, hi]
      have gA0 : toExt a0 = tensorRead low high (2 ^ k) (2 * u.val) := by
        rw [ha0, he, he1',
          ← List.getD_eq_getElem low.val cpoly.field.Ext4.ZERO hi1b,
          ← List.getD_eq_getElem high.val cpoly.field.Ext4.ZERO hi2b]
        simp only [tensorRead, hi1, hi2, hi, hl]
      have gA1 : toExt a1 = tensorRead low high (2 ^ k) (2 * u.val + 1) := by
        rw [ha1, he2', he3,
          ← List.getD_eq_getElem low.val cpoly.field.Ext4.ZERO hi4b,
          ← List.getD_eq_getElem high.val cpoly.field.Ext4.ZERO hi6b]
        simp only [tensorRead, hi4, hi6, hi5, hi3, hi, hl]
      refine ⟨by scalar_tac, hRc01, hRc11, hRc21, ?_, ?_, ?_, by scalar_tac⟩
      · rw [hc01, he0, hu1, Finset.sum_range_succ, hp0, gW0, gA0, qC0]
      · rw [hc11, he1, hu1, Finset.sum_range_succ, he11, he10, hp1, hp2, he8, he9,
          gW1, gA1, hp0, gW0, gA0, gW1', gW0', qC1, qC0, qC2]
      · rw [hc21, he2, hu1, Finset.sum_range_succ, hp2, he8, he9,
          gW1', gW0', gA1, gA0, qC2]
    · rw [if_neg hlt, WP.spec_ok]
      dsimp only
      have hueq : u.val = 2 ^ κ := by
        have : half.val ≤ u.val := by scalar_tac
        omega
      exact ⟨hD0, hD1, hD2, by rw [he0, hueq], by rw [he1, hueq], by rw [he2, hueq]⟩
  · exact ⟨hyb, hR0, hR1, hR2, hv0, hv1, hv2⟩
...
theorem round_poly_alpha_split_blocks_spec {j k κ : ℕ} (hjk : j + (k + 1) = κ + 1)
  ...
  have hco : ∀ t : ℕ, t < 3 → (toRaw v3).coeff t
    ...
    rw [toRaw_coeff_of_lt v3 (by rw [hv3l]; exact ht),
      ← List.getD_eq_getElem (l := v3.val) (d := cpoly.field.Ext4.ZERO) (by rw [hv3l]; exact ht)]
  rw [hco 0 (by norm_num), hco 1 (by norm_num), hco 2 (by norm_num),
    hg0, hg1, hg2, hc0, hc1, hc2, linSumAlphaSplit_eq_sum_range hjk]
  simp only [Finset.sum_mul]
  rw [← Finset.sum_add_distrib, ← Finset.sum_add_distrib]
  refine Finset.sum_congr rfl (fun t _ => ?_)
  simp only [qC0, qC1, qC2, evenF, oddF]
...
/-! ##### Card T41b1: the coefficients directly, at round 0

`round_poly_alpha_base_split` used to evaluate the three node values above and
interpolate them. Card T41b1 gives it `round_poly_alpha_split`'s
direct-coefficient loop (card T38) instead, with the `w̃` entries still in `Fp`:
the per-pair coefficients are the same `qC0`/`qC1`/`qC2` over the readers
`evenPhiF`/`oddPhiF` below, and every product with a `w̃` factor is the mixed
`Fp × Ext4` multiply (`fp_ext_mul_spec`), the difference `w₁ − w₀` taken in
`ZMod q` first and embedded through `phiF`'s `map_sub`. The node-value items
above stay in the Rust and keep their specs; only the headline's proof moved,
and its statement did not.

Card T41b2 moved that loop, verbatim, into `round_poly_alpha_base_split_direct`
(the path for an odd or empty `low`), and its two specs with it: the loop spec
renamed, proof unchanged, and the former headline's statement and proof as
`round_poly_alpha_base_split_direct_spec`. The headline itself now follows the
block traversal below. -/

/-- The even-indexed entry of a base-field table, embedded in `F`. -/
def evenPhiF (v : alloc.vec.Vec cpoly.field.Fp) (y : ℕ) : F :=
  phiF (coeffK v (2 * y))

/-- The odd-indexed entry of a base-field table, embedded in `F`. -/
def oddPhiF (v : alloc.vec.Vec cpoly.field.Fp) (y : ℕ) : F :=
  phiF (coeffK v (2 * y + 1))

/-- The `y` loop of `round_poly_alpha_base_split_direct`:
[`round_poly_alpha_split_pairs_loop_spec`] with the `w̃` readers `evenF`/`oddF`
replaced by their base-field twins `evenPhiF`/`oddPhiF`. -/
theorem round_poly_alpha_base_split_direct_loop_spec {j k κ : ℕ} (hjk : j + k = κ + 1)
    (w : alloc.vec.Vec cpoly.field.Fp) (low high : alloc.vec.Vec cpoly.field.Ext4)
    (half l : Std.Usize) (c0 c1 c2 : cpoly.field.Ext4) (y : Std.Usize)
    (hw : WfEvalsFp (κ + 1) w) (hlow : WfEvals k low) (hhigh : WfEvals j high)
    (hhalfv : half.val = 2 ^ κ) (hl : l.val = 2 ^ k) (hyb : y.val ≤ 2 ^ κ)
    (hR0 : Reduced c0) (hR1 : Reduced c1) (hR2 : Reduced c2)
    (hv0 : toExt c0 = ∑ t ∈ Finset.range y.val,
      qC0 (evenPhiF w) (fun u => tensorRead low high (2 ^ k) (2 * u)) t)
    (hv1 : toExt c1 = ∑ t ∈ Finset.range y.val,
      qC1 (evenPhiF w) (oddPhiF w) (fun u => tensorRead low high (2 ^ k) (2 * u))
        (fun u => tensorRead low high (2 ^ k) (2 * u + 1)) t)
    (hv2 : toExt c2 = ∑ t ∈ Finset.range y.val,
      qC2 (evenPhiF w) (oddPhiF w) (fun u => tensorRead low high (2 ^ k) (2 * u))
        (fun u => tensorRead low high (2 ^ k) (2 * u + 1)) t) :
    sumcheck.round_poly_alpha_base_split_direct_loop w low high half l c0 c1 c2 y
      ⦃ z => Reduced z.1 ∧ Reduced z.2.1 ∧ Reduced z.2.2
        ∧ toExt z.1 = ∑ t ∈ Finset.range (2 ^ κ),
            qC0 (evenPhiF w) (fun u => tensorRead low high (2 ^ k) (2 * u)) t
        ∧ toExt z.2.1 = ∑ t ∈ Finset.range (2 ^ κ),
  ...
  have htwo : (2 : ℕ) ^ k * 2 ^ j = 2 ^ (κ + 1) := by rw [← pow_add]; congr 1; omega
  have hmax := usize_max_ge
  rw [sumcheck.round_poly_alpha_base_split_direct_loop]
  apply loop.spec_decr_nat (fun s => 2 ^ κ - s.2.2.2.val)
    (fun s => s.2.2.2.val ≤ 2 ^ κ ∧ Reduced s.1 ∧ Reduced s.2.1 ∧ Reduced s.2.2.1
      ∧ toExt s.1 = ∑ t ∈ Finset.range s.2.2.2.val,
          qC0 (evenPhiF w) (fun u => tensorRead low high (2 ^ k) (2 * u)) t
  ...
  · rintro ⟨d0, d1, d2, u⟩ ⟨hu, hD0, hD1, hD2, he0, he1, he2⟩
    dsimp only at hu hD0 hD1 hD2 he0 he1 he2
    simp only [sumcheck.round_poly_alpha_base_split_direct_loop.body]
    by_cases hlt : u < half
    · rw [if_pos hlt]
      have hult : u.val < 2 ^ κ := by
        have : u.val < half.val := by scalar_tac
      ...
      step as ⟨a1, hRa1, ha1⟩
      step as ⟨w0, hw0⟩
      have hRw0 : Red w0 := hw0 ▸ hwred _ (List.getElem_mem hib)
      step as ⟨i7, hi7⟩
      have hi7b : i7.val < w.val.length := by rw [hi7, hi]; omega
      step as ⟨w1, hw1⟩
      have hRw1 : Red w1 := hw1 ▸ hwred _ (List.getElem_mem hi7b)
      step with fp_ext_mul_spec w0 a0 hRw0 hRa0 as ⟨p0, hRp0, hp0⟩
      step with fp_ext_mul_spec w1 a1 hRw1 hRa1 as ⟨p1, hRp1, hp1⟩
      step with fp_sub_spec w1 w0 hRw1 hRw0 as ⟨f, hRf, hf⟩
      step as ⟨e4, hRe4, he4⟩
      step with fp_ext_mul_spec f e4 hRf hRe4 as ⟨p2, hRp2, hp2⟩
      step as ⟨c01, hRc01, hc01⟩
      step as ⟨e5, hRe5, he5⟩
      step as ⟨e6, hRe6, he6⟩
      step as ⟨c11, hRc11, hc11⟩
      step as ⟨c21, hRc21, hc21⟩
      step as ⟨u1, hu1⟩
      have hp0' : toExt p0 = phiF (toK w0) * toExt a0 := by rw [hp0, phiF_apply]
      have hp1' : toExt p1 = phiF (toK w1) * toExt a1 := by rw [hp1, phiF_apply]
      have hp2' : toExt p2 = phiF (toK f) * toExt e4 := by rw [hp2, phiF_apply]
      have gW0 : phiF (toK w0) = evenPhiF w u.val := by
        rw [hw0, evenPhiF, coeffK, ← List.getD_eq_getElem w.val cpoly.field.Fp.ZERO hib, hi]
      have gW1 : phiF (toK w1) = oddPhiF w u.val := by
        rw [hw1, oddPhiF, coeffK, ← List.getD_eq_getElem w.val cpoly.field.Fp.ZERO hi7b,
          hi7, hi]
      have gWd : phiF (toK f) = oddPhiF w u.val - evenPhiF w u.val := by
        rw [hf, map_sub, gW0, gW1]
      have gA0 : toExt a0 = tensorRead low high (2 ^ k) (2 * u.val) := by
        rw [ha0, he, he1',
          ← List.getD_eq_getElem low.val cpoly.field.Ext4.ZERO hi1b,
          ← List.getD_eq_getElem high.val cpoly.field.Ext4.ZERO hi2b]
      ...
      refine ⟨by scalar_tac, hRc01, hRc11, hRc21, ?_, ?_, ?_, by scalar_tac⟩
      · rw [hc01, he0, hu1, Finset.sum_range_succ, hp0', gW0, gA0, qC0]
      · rw [hc11, he1, hu1, Finset.sum_range_succ, he6, he5, hp1', hp2', he4,
          gW1, gA1, hp0', gW0, gA0, gWd, qC1, qC0, qC2]
      · rw [hc21, he2, hu1, Finset.sum_range_succ, hp2', he4, gWd, gA1, gA0, qC2]
    · rw [if_neg hlt, WP.spec_ok]
      dsimp only
      have hueq : u.val = 2 ^ κ := by
        have : half.val ≤ u.val := by scalar_tac
...
/-- `round_poly_alpha_base_split_direct` builds the three coefficients of
`linSumAlpha` at the embedded table against the tensor table directly (card
T41b1), and so equals it everywhere -- `round_poly_alpha_split_spec`'s proof
with the `w̃` readers in the base field. Until card T41b2 this was
`round_poly_alpha_base_split_spec`, statement and proof. -/
theorem round_poly_alpha_base_split_direct_spec {j k κ : ℕ} (hjk : j + k = κ + 1)
    (w : alloc.vec.Vec cpoly.field.Fp) (low high : alloc.vec.Vec cpoly.field.Ext4)
    (hw : WfEvalsFp (κ + 1) w) (hlow : WfEvals k low) (hhigh : WfEvals j high) :
    sumcheck.round_poly_alpha_base_split_direct w low high
      ⦃ out => out.val.length = 3 ∧ VecReduced out ∧
        ∀ x : F, CPolynomial.eval x (toUni out) =
          linSumAlpha (phiF ∘ tableFnFp (m := κ + 1) w)
            (reidx hjk (tensorTable (tableFn (m := k) low) (tableFn (m := j) high)))
  ...
  have hRZ : Reduced cpoly.field.Ext4.ZERO := reduced_ZERO
  have hmax := usize_max_ge
  rw [sumcheck.round_poly_alpha_base_split_direct]
  step as ⟨half, hhalf⟩
  have hlenw : (alloc.vec.Vec.len w).val = w.val.length := by simp
  have hhalfv : half.val = 2 ^ κ := by
    rw [hhalf, hlenw, hwlen, pow_succ]; omega
  step with round_poly_alpha_base_split_direct_loop_spec hjk w low high half
    (alloc.vec.Vec.len low)
    cpoly.field.Ext4.ZERO cpoly.field.Ext4.ZERO cpoly.field.Ext4.ZERO 0#usize
  ...
  have hco : ∀ t : ℕ, t < 3 → (toRaw v3).coeff t
    ...
    rw [toRaw_coeff_of_lt v3 (by rw [hv3l]; exact ht),
      ← List.getD_eq_getElem (l := v3.val) (d := cpoly.field.Ext4.ZERO) (by rw [hv3l]; exact ht)]
  rw [hco 0 (by norm_num), hco 1 (by norm_num), hco 2 (by norm_num),
    hg0, hg1, hg2, hc0, hc1, hc2, linSumAlphaSplitFp_eq_sum_range_ext hjk]
  simp only [Finset.sum_mul]
  rw [← Finset.sum_add_distrib, ← Finset.sum_add_distrib]
  refine Finset.sum_congr rfl (fun t _ => ?_)
  simp only [qC0, qC1, qC2, evenPhiF, oddPhiF]
  ring

/-! ##### Card T41b2: the pairs visited block by block, at round 0

`round_poly_alpha_base_split` now dispatches on `l = low.len()` the way
`round_poly_alpha_split` does since card T41a, except that `l = 1` is odd and so
takes the per-pair path: an odd or empty `l` goes to
`round_poly_alpha_base_split_direct` (card T41b1's loop, above), an even one to
`round_poly_alpha_base_split_blocks`, which sums each high block against `low`
alone (`alpha_block_sums_base`) and scales the three sums once by the block's
`high` entry. The specs are card T41a's block specs with the `w̃` readers
`evenF`/`oddF` replaced by `evenPhiF`/`oddPhiF`, every product with a `w̃` factor
the mixed `Fp × Ext4` multiply (`fp_ext_mul_spec`) and `w₁ − w₀` one `Fp`
subtraction (`fp_sub_spec`), as in the direct loop. The index facts
(`tensorRead_block`) and the regrouping (`qC_block_scale`) are over abstract
readers and carry over unchanged. -/

/-- The `t` loop of `alpha_block_sums_base`: `alpha_block_sums_loop_spec` with
the `w̃` readers in the base field -- the three sums over the block's first `t`
pairs, `w` read at the pairs `y₀ + t` (`base = 2 · y₀`, a ghost) and `low` at
`2t`, `2t + 1`. -/
theorem alpha_block_sums_base_loop_spec (w : alloc.vec.Vec cpoly.field.Fp)
    (low : alloc.vec.Vec cpoly.field.Ext4)
    (base cnt : Std.Usize) (y0 : ℕ) (s0 s1 s2 : cpoly.field.Ext4) (t : Std.Usize)
    (hwred : ∀ a ∈ w.val, Red a) (hlowred : VecReduced low)
    (hbase : base.val = 2 * y0) (hwb : 2 * (y0 + cnt.val) ≤ w.val.length)
    (hlb : 2 * cnt.val ≤ low.val.length) (htb : t.val ≤ cnt.val)
    (hR0 : Reduced s0) (hR1 : Reduced s1) (hR2 : Reduced s2)
    (hv0 : toExt s0 = ∑ u ∈ Finset.range t.val,
      qC0 (fun u => evenPhiF w (y0 + u)) (evenF low) u)
    (hv1 : toExt s1 = ∑ u ∈ Finset.range t.val,
      qC0 (fun u => oddPhiF w (y0 + u)) (oddF low) u)
    (hv2 : toExt s2 = ∑ u ∈ Finset.range t.val,
      qC2 (fun u => evenPhiF w (y0 + u)) (fun u => oddPhiF w (y0 + u))
        (evenF low) (oddF low) u) :
    sumcheck.alpha_block_sums_base_loop w low base cnt s0 s1 s2 t
      ⦃ z => Reduced z.1 ∧ Reduced z.2.1 ∧ Reduced z.2.2
        ∧ toExt z.1 = ∑ u ∈ Finset.range cnt.val,
            qC0 (fun u => evenPhiF w (y0 + u)) (evenF low) u
        ∧ toExt z.2.1 = ∑ u ∈ Finset.range cnt.val,
  ...
  have hwmax : w.val.length ≤ Usize.max := w.property
  have hlmax : low.val.length ≤ Usize.max := low.property
  rw [sumcheck.alpha_block_sums_base_loop]
  apply loop.spec_decr_nat (fun s => cnt.val - s.2.2.2.val)
    (fun s => s.2.2.2.val ≤ cnt.val ∧ Reduced s.1 ∧ Reduced s.2.1 ∧ Reduced s.2.2.1
      ∧ toExt s.1 = ∑ u ∈ Finset.range s.2.2.2.val,
          qC0 (fun u => evenPhiF w (y0 + u)) (evenF low) u
  ...
  · rintro ⟨d0, d1, d2, u⟩ ⟨hu, hD0, hD1, hD2, hS0, hS1, hS2⟩
    dsimp only at hu hD0 hD1 hD2 hS0 hS1 hS2
    simp only [sumcheck.alpha_block_sums_base_loop.body]
    by_cases hlt : u < cnt
    · rw [if_pos hlt]
      have hult : u.val < cnt.val := by scalar_tac
      have hwu : 2 * y0 + 2 * u.val + 1 < w.val.length := by omega
      ...
      have hi1b : i1.val < w.val.length := by rw [hi1, hi, hkk, hbase]; omega
      step as ⟨w1, hw1⟩
      have hRw1 : Red w1 := hw1 ▸ hwred _ (List.getElem_mem hi1b)
      have hkb : kk.val < low.val.length := by rw [hkk]; omega
      step as ⟨l0, hl0⟩
      have hRl0 : Reduced l0 := hl0 ▸ hlowred _ (List.getElem_mem hkb)
      step as ⟨i2, hi2⟩
      ...
      step as ⟨l1, hl1⟩
      have hRl1 : Reduced l1 := hl1 ▸ hlowred _ (List.getElem_mem hi2b)
      step with fp_ext_mul_spec w0 l0 hRw0 hRl0 as ⟨p0, hRp0, hp0⟩
      step with fp_ext_mul_spec w1 l1 hRw1 hRl1 as ⟨p1, hRp1, hp1⟩
      step with fp_sub_spec w1 w0 hRw1 hRw0 as ⟨f, hRf, hf⟩
      step as ⟨e, hRe, he⟩
      step with fp_ext_mul_spec f e hRf hRe as ⟨p2, hRp2, hp2⟩
      step as ⟨s01, hRs01, hs01⟩
      step as ⟨s11, hRs11, hs11⟩
      step as ⟨s21, hRs21, hs21⟩
      step as ⟨t1, ht1⟩
      have hp0' : toExt p0 = phiF (toK w0) * toExt l0 := by rw [hp0, phiF_apply]
      have hp1' : toExt p1 = phiF (toK w1) * toExt l1 := by rw [hp1, phiF_apply]
      have hp2' : toExt p2 = phiF (toK f) * toExt e := by rw [hp2, phiF_apply]
      have gW0 : phiF (toK w0) = evenPhiF w (y0 + u.val) := by
        rw [hw0, evenPhiF, coeffK, ← List.getD_eq_getElem w.val cpoly.field.Fp.ZERO hib, hi,
          hkk, hbase, mul_add]
      have gW1 : phiF (toK w1) = oddPhiF w (y0 + u.val) := by
        rw [hw1, oddPhiF, coeffK, ← List.getD_eq_getElem w.val cpoly.field.Fp.ZERO hi1b, hi1,
          hi, hkk, hbase, mul_add]
      have gWd : phiF (toK f) = oddPhiF w (y0 + u.val) - evenPhiF w (y0 + u.val) := by
        rw [hf, map_sub, gW0, gW1]
      have gL0 : toExt l0 = evenF low u.val := by
        rw [hl0, evenF, ← List.getD_eq_getElem low.val cpoly.field.Ext4.ZERO hkb, hkk]
      have gL1 : toExt l1 = oddF low u.val := by
      ...
      · rw [hs01, hS0, ht1, Finset.sum_range_succ, hp0', gW0, gL0, qC0]
      · rw [hs11, hS1, ht1, Finset.sum_range_succ, hp1', gW1, gL1, qC0]
      · rw [hs21, hS2, ht1, Finset.sum_range_succ, hp2', he, gWd, gL1, gL0, qC2]
    · rw [if_neg hlt, WP.spec_ok]
      dsimp only
      have hueq : u.val = cnt.val := by
        have : cnt.val ≤ u.val := by scalar_tac
...
/-- `alpha_block_sums_base`: one block's three sums against the `low` factor
alone, the pairs `y₀ + t` (`t < cnt`) of the base-field `w` against `low[2t]`,
`low[2t + 1]`. -/
theorem alpha_block_sums_base_spec (w : alloc.vec.Vec cpoly.field.Fp)
    (low : alloc.vec.Vec cpoly.field.Ext4)
    (base cnt : Std.Usize) (y0 : ℕ)
    (hwred : ∀ a ∈ w.val, Red a) (hlowred : VecReduced low)
    (hbase : base.val = 2 * y0) (hwb : 2 * (y0 + cnt.val) ≤ w.val.length)
    (hlb : 2 * cnt.val ≤ low.val.length) :
    sumcheck.alpha_block_sums_base w low base cnt
      ⦃ z => Reduced z.1 ∧ Reduced z.2.1 ∧ Reduced z.2.2
        ∧ toExt z.1 = ∑ u ∈ Finset.range cnt.val,
            qC0 (fun u => evenPhiF w (y0 + u)) (evenF low) u
        ∧ toExt z.2.1 = ∑ u ∈ Finset.range cnt.val,
        ...
        ∧ toExt z.2.2 = ∑ u ∈ Finset.range cnt.val,
            qC2 (fun u => evenPhiF w (y0 + u)) (fun u => oddPhiF w (y0 + u))
              (evenF low) (oddF low) u ⦄ := by
  rw [sumcheck.alpha_block_sums_base]
  exact alpha_block_sums_base_loop_spec w low base cnt y0 _ _ _ 0#usize hwred hlowred hbase
    hwb hlb (by simp) reduced_ZERO reduced_ZERO reduced_ZERO
    (by simp [toExt_ZERO]) (by simp [toExt_ZERO]) (by simp [toExt_ZERO])

/-- The block loop of `round_poly_alpha_base_split_blocks`:
`round_poly_alpha_split_blocks_loop_spec` with the `w̃` readers in the base
field -- T38's three coefficient sums over the tensor read, carried over the
first `y₀ = b · 2 ^ k` pairs (`b` whole high blocks of the even low factor
`2 ^ (k + 1)`), which is `round_poly_alpha_base_split_direct_loop_spec`'s
invariant at the block boundaries. -/
theorem round_poly_alpha_base_split_blocks_loop_spec {j k κ : ℕ}
    (hjk : j + (k + 1) = κ + 1)
    (w : alloc.vec.Vec cpoly.field.Fp) (low high : alloc.vec.Vec cpoly.field.Ext4)
    (half lh : Std.Usize) (c0 c1 c2 : cpoly.field.Ext4) (y0 b : Std.Usize)
    (hw : WfEvalsFp (κ + 1) w) (hlow : WfEvals (k + 1) low) (hhigh : WfEvals j high)
    (hhalfv : half.val = 2 ^ κ) (hlh : lh.val = 2 ^ k)
    (hyb : y0.val = b.val * 2 ^ k) (hbb : b.val ≤ 2 ^ j)
    (hR0 : Reduced c0) (hR1 : Reduced c1) (hR2 : Reduced c2)
    (hv0 : toExt c0 = ∑ t ∈ Finset.range y0.val,
      qC0 (evenPhiF w) (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u)) t)
    (hv1 : toExt c1 = ∑ t ∈ Finset.range y0.val,
      qC1 (evenPhiF w) (oddPhiF w) (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u))
        (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u + 1)) t)
    (hv2 : toExt c2 = ∑ t ∈ Finset.range y0.val,
      qC2 (evenPhiF w) (oddPhiF w) (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u))
        (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u + 1)) t) :
    sumcheck.round_poly_alpha_base_split_blocks_loop w low high half lh c0 c1 c2 y0 b
      ⦃ z => Reduced z.1 ∧ Reduced z.2.1 ∧ Reduced z.2.2
        ∧ toExt z.1 = ∑ t ∈ Finset.range (2 ^ κ),
            qC0 (evenPhiF w) (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u)) t
        ∧ toExt z.2.1 = ∑ t ∈ Finset.range (2 ^ κ),
  ...
  have htwo : (2 : ℕ) ^ κ = 2 ^ j * 2 ^ k := by rw [← pow_add]; congr 1; omega
  have hpk : 0 < 2 ^ k := by positivity
  rw [sumcheck.round_poly_alpha_base_split_blocks_loop]
  apply loop.spec_decr_nat (fun s => 2 ^ κ - s.2.2.2.1.val)
    (fun s => s.2.2.2.1.val = s.2.2.2.2.val * 2 ^ k ∧ s.2.2.2.2.val ≤ 2 ^ j
      ∧ Reduced s.1 ∧ Reduced s.2.1 ∧ Reduced s.2.2.1
      ∧ toExt s.1 = ∑ t ∈ Finset.range s.2.2.2.1.val,
  ...
  · rintro ⟨d0, d1, d2, y, bb⟩ ⟨hy, hb, hD0, hD1, hD2, hS0, hS1, hS2⟩
    dsimp only at hy hb hD0 hD1 hD2 hS0 hS1 hS2
    simp only [sumcheck.round_poly_alpha_base_split_blocks_loop.body]
    by_cases hlt : y < half
    · rw [if_pos hlt]
      have hylt : y.val < 2 ^ κ := by
        have : y.val < half.val := by scalar_tac
      ...
      rw [hcnteq, bind_tc_ok]
      step as ⟨i, hi⟩
      step with alpha_block_sums_base_spec w low i cnt y.val hwred hlowred hi
        (by rw [hcntv]; exact hwy) (by rw [hcntv, hlowlen, pow_succ]; omega)
        as ⟨s, hRs0, hRs1, hRs2, hs0, hs1, hs2⟩
      rw [hcntv] at hs0 hs1 hs2
      step as ⟨h, hh⟩
      ...
      have hA1 : ∀ u < 2 ^ k,
        ...
        rw [gH, hy]
        exact (tensorRead_block low high bb.val u hu).2
      obtain ⟨hB0, hB1, hB2⟩ := qC_block_scale (evenPhiF w) (oddPhiF w)
        (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u))
        (fun u => tensorRead low high (2 ^ (k + 1)) (2 * u + 1))
        (evenF low) (oddF low) (toExt h) y.val (2 ^ k) hA0 hA1
      have hy1v : y1.val = y.val + 2 ^ k := by rw [hy1, hcntv]
...
/-- `round_poly_alpha_base_split_blocks` builds T38's three coefficients block
by block for an even low factor `2 ^ (k + 1)`, with the `w̃` table in the base
field, and so equals `linSumAlpha` at the embedded table against the tensor
table everywhere: `round_poly_alpha_base_split_direct_spec`'s conclusion at
`k + 1`, with the same tail. -/
theorem round_poly_alpha_base_split_blocks_spec {j k κ : ℕ} (hjk : j + (k + 1) = κ + 1)
    (w : alloc.vec.Vec cpoly.field.Fp) (low high : alloc.vec.Vec cpoly.field.Ext4)
    (hw : WfEvalsFp (κ + 1) w) (hlow : WfEvals (k + 1) low) (hhigh : WfEvals j high) :
    sumcheck.round_poly_alpha_base_split_blocks w low high
      ⦃ out => out.val.length = 3 ∧ VecReduced out ∧
        ∀ x : F, CPolynomial.eval x (toUni out) =
          linSumAlpha (phiF ∘ tableFnFp (m := κ + 1) w)
            (reidx hjk (tensorTable (tableFn (m := k + 1) low) (tableFn (m := j) high)))
  ...
  have hRZ : Reduced cpoly.field.Ext4.ZERO := reduced_ZERO
  have hmax := usize_max_ge
  rw [sumcheck.round_poly_alpha_base_split_blocks]
  step as ⟨half, hhalf⟩
  have hlenw : (alloc.vec.Vec.len w).val = w.val.length := by simp
  have hhalfv : half.val = 2 ^ κ := by
    rw [hhalf, hlenw, hwlen, pow_succ]; omega
  ...
  have hlhv : lh.val = 2 ^ k := by
    rw [hlh, hlenl, hlowlen, pow_succ]; omega
  step with round_poly_alpha_base_split_blocks_loop_spec hjk w low high half lh
    cpoly.field.Ext4.ZERO cpoly.field.Ext4.ZERO cpoly.field.Ext4.ZERO 0#usize 0#usize
    hw hlow hhigh hhalfv hlhv (by simp) (by simp) hRZ hRZ hRZ
    (by simp [toExt_ZERO]) (by simp [toExt_ZERO]) (by simp [toExt_ZERO])
    as ⟨c0, c1, c2, hR0, hR1, hR2, hc0, hc1, hc2⟩
...
/-- `round_poly_alpha_base_split` builds the three coefficients of
`linSumAlpha` at the embedded table against the tensor table (cards T41b1,
T41b2), and so equals it everywhere. Card T41b2 dispatches it on
`low.len() = 2 ^ k`: `k = 0` gives the odd `l = 1`, the per-pair path
(`round_poly_alpha_base_split_direct_spec`); `k ≥ 1` gives an even, nonzero
`l`, the block traversal (`round_poly_alpha_base_split_blocks_spec`). The
empty `low` is unreachable under the spec's `2 ^ k`. Statement unchanged. -/
theorem round_poly_alpha_base_split_spec {j k κ : ℕ} (hjk : j + k = κ + 1)
  ...
  rcases Nat.eq_zero_or_pos k with hk0 | hkpos
  · subst hk0
    have hi1 : i.val = 1 := by rw [hi, hlenl, hlowlen, pow_zero]; norm_num
    rw [if_pos (by scalar_tac)]
    exact round_poly_alpha_base_split_direct_spec hjk w low high hw hlow hhigh
  · obtain ⟨k', rfl⟩ : ∃ k', k = k' + 1 := ⟨k - 1, by omega⟩
    have h2 : 2 ≤ low.val.length := by
      rw [hlowlen, pow_succ]
      have : 0 < 2 ^ k' := by positivity
    ...
    have hi0 : i.val = 0 := by
      rw [hi, hlenl, hlowlen, pow_succ, Nat.mul_mod_left]
    rw [if_neg (by scalar_tac), if_neg (by scalar_tac)]
    exact round_poly_alpha_base_split_blocks_spec hjk w low high hw hlow hhigh
...
theorem honest_compute_g_base_split_spec {n μ M m₁ dRows j k : ℕ}
  ...
  · refine cpoly_eq_of_eval_eq (d := 2) (by norm_num) _ _
    ...
    refine Finset.sum_congr rfl fun z _ => ?_
    rw [InnerOuter.eval_sumcheckPolyAlpha, hfoldw x z, hfolda x z]
/-- `eq_suffix_table_spec` with the round index named as a natural number: the
same statement at `n` for any `i` with `i.val = n`, which is how a literal round
index (`eq_suffix_table(tau0, 1)`) reaches a statement indexed by `1 : ℕ`. -/
theorem eq_suffix_table_spec_at {m₀ : ℕ} (tau0 : alloc.vec.Vec cpoly.field.Ext4)
    (i : Std.Usize) (n : ℕ) (hin : i.val = n)
    (h0 : WfPoint m₀ tau0) (hi : n < Usize.max)
    (hsz : 2 ^ (m₀ - n - 1) ≤ Usize.max) :
    sumcheck.eq_suffix_table tau0 i
      ⦃ out => WfEvals (m₀ - n - 1) out ∧
        toEvals (m := m₀ - n - 1) out =
          CMlPolynomialEval.lagrangeBasis (Vector.ofFn fun k : Fin (m₀ - n - 1) =>
            toPoint (m := m₀) tau0 ⟨n + 1 + k.val, by omega⟩) ⦄ := by
  subst hin
  exact eq_suffix_table_spec tau0 i h0 hi hsz

/-- `honest_compute_g_fold1_split` computes `honestComputeG` at round `1` (card
T46b): `honest_compute_g_split_spec`'s statement at `i = 1`, word for word in
its conclusion, with the zero side's table read off the round-0 base table.

**Statement delta from `honest_compute_g_split_spec` at `i = 1`:** the usize
round index is gone (the Rust hard-codes `1`), and three hypotheses are added
for the zero side's inputs: `hfp : WfEvalsFp (M + 1) w_fp`, `ha0 : Reduced a0`
and `hwfold`, which says `w_tab` *is* `w_fp` folded at `a0` -- exactly what
`eval_mle_layer_base_spec` delivers for the table the peeled round 0 hands on.
`w_tab` is still what the linear side reads. The proof is the twin's with
`round_poly_zero_fold1_spec` in place of `round_poly_zero_spec`, `hwfold`
turning its table back into `tableFn w_tab`. -/
theorem honest_compute_g_fold1_split_spec {n μ M m₁ dRows j k : ℕ}
    (stmt : sumcheck.RoundStatement) (w_fp : alloc.vec.Vec cpoly.field.Fp)
    (a0 : cpoly.field.Ext4) (w_tab low high : alloc.vec.Vec cpoly.field.Ext4)
    (ss : InnerOuter.NestedRoundStatement Φ (PolyVec (Rq Φ) dRows) F n μ (M + 1) m₁ 1)
    (sw : InnerOuter.LiftedWitness Φ μ n)
    (hs : RepRoundStmt (n := n) (μ := μ) (m₀ := M + 1) (m₁ := m₁) (i := 1) (dRows := dRows)
      stmt ss)
    (hi : 1 < M + 1) (hjk : j + k = M + 1 - 1)
    (hw : WfEvals (M + 1 - 1) w_tab)
    (hfp : WfEvalsFp (M + 1) w_fp) (ha0 : Reduced a0)
    (hwfold : ∀ y : Fin (2 ^ M),
      tableFn (m := M) w_tab y = fold (phiF ∘ tableFnFp (m := M + 1) w_fp) (toExt a0) y)
    (hlow : WfEvals k low) (hhigh : WfEvals j high)
    (hwv : ∀ y : Fin (M + 1 - 1) → Fin 2,
      tableFn (m := M + 1 - 1) w_tab (finFunctionFinEquiv y) =
        InnerOuter.wTableMleEval Φ (M + 1) phiF 16 sw
          (InnerOuter.hypercubePoint (M + 1) 1 ss.challenges y))
    (hav : ∀ y : Fin (M + 1 - 1) → Fin 2,
      reidx hjk (tensorTable (tableFn (m := k) low) (tableFn (m := j) high))
          (finFunctionFinEquiv y) =
        (InnerOuter.cMultilinearExtension (M + 1)
          (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16 ss.zc.rlin ss.zc.α ss.zc.τα)).eval
          (InnerOuter.hypercubePoint (M + 1) 1 ss.challenges y)) :
    sumcheck.honest_compute_g_fold1_split stmt w_fp a0 w_tab low high
      ⦃ out => RepRoundMsg out
        (InnerOuter.honestComputeG Φ m₁ 16 (by norm_num) phiF 1 hi ss sw) ⦄ := by
  obtain ⟨hzc, hWc, hRt0, hRta, hc, ht0, hta⟩ := hs
  obtain ⟨hr, hWt, hWa, hW0, hW1, htv, hav', h0v, h1v⟩ := hzc
  obtain ⟨h0len, h0red⟩ := hW0
  have hM1 : M + 1 ≤ Usize.max := by
    have := stmt.zc.tau0.property
    omega
  have hkk : M + 1 - 1 = (M + 1 - 1 - 1) + 1 := by omega
  have hw' : WfEvals ((M + 1 - 1 - 1) + 1) w_tab := by rwa [hkk] at hw
  have hjk' : j + k = (M + 1 - 1 - 1) + 1 := by omega
  have hcube : 2 ^ (M + 1 - 1) ≤ Usize.max := by
    have hlen := hw.1
    have := w_tab.property
    omega
  have hfp' : WfEvalsFp ((M + 1 - 1 - 1) + 1 + 1) w_fp := by
    rwa [show (M + 1 - 1 - 1) + 1 + 1 = M + 1 by omega]
  have hwf : ∀ t, t < 2 ^ M → toExt (w_tab.val.getD t cpoly.field.Ext4.ZERO)
      = (1 - toExt a0) * phiF (coeffK w_fp (2 * t)) + toExt a0 * phiF (coeffK w_fp (2 * t + 1)) := by
    intro t ht
    have h := hwfold ⟨t, ht⟩
    rw [tableFn_apply] at h
    rw [h]
    rfl
  have h1v : (1#usize : Std.Usize).val = 1 := by simp
  simp only [sumcheck.honest_compute_g_fold1_split, sumcheck.RoundStatement.impl.zc,
    sumcheck.NestedZeroCheckStmt.impl.tau0, sumcheck.RoundStatement.impl.challenges, bind_tc_ok]
  step with eq_prefix_spec (m₀ := M + 1) (i := 1) stmt.zc.tau0 stmt.challenges
    ⟨h0len, h0red⟩ hWc (by omega) as ⟨pref, hRpref, hprefv⟩
  step with eq_suffix_table_spec_at (m₀ := M + 1) stmt.zc.tau0 1#usize 1 h1v ⟨h0len, h0red⟩
    (by omega)
    (by
      have hmono : (2 : ℕ) ^ (M + 1 - 1 - 1) ≤ 2 ^ (M + 1 - 1) :=
        Nat.pow_le_pow_right (by omega) (by omega)
      omega)
      as ⟨suffix, hWsuf, hsufv⟩
  step with round_poly_zero_fold1_spec (k := M + 1 - 1 - 1) w_fp a0 suffix hfp' hWsuf ha0
    as ⟨inner, hinlen, hinred, hinval0⟩
  have hinval : ∀ x : F, CPolynomial.eval x (toUni inner)
      = rangeSumZero (tableFn (m := (M + 1 - 1 - 1) + 1) w_tab)
          (tableFn (m := M + 1 - 1 - 1) suffix) x := by
    intro x
    have hfun : fold (phiF ∘ tableFnFp (m := (M + 1 - 1 - 1) + 1 + 1) w_fp) (toExt a0)
        = tableFn (m := (M + 1 - 1 - 1) + 1) w_tab := by
      funext y
      have hy : y.val < 2 ^ M := by
        have h2 : (2 : ℕ) ^ ((M + 1 - 1 - 1) + 1) = 2 ^ M := by
          rw [show (M + 1 - 1 - 1) + 1 = M by omega]
        exact lt_of_lt_of_eq y.isLt h2
      rw [tableFn_apply, hwf y.val hy]
      rfl
    rw [hinval0 x, hfun]
  step as ⟨e, he⟩
  have hRe : Reduced e := he ▸ h0red _ (List.getElem_mem (by omega))
  ...
  step with round_poly_alpha_split_spec hjk' w_tab low high hw' hlow hhigh
    as ⟨ga, hgalen, hgared, hgaval⟩
  have hwv2 : ∀ y : Fin (M + 1 - 1) → Fin 2,
      tableFn (m := M + 1 - 1) w_tab (finFunctionFinEquiv y)
        = (InnerOuter.cMultilinearExtension (M + 1)
            (InnerOuter.wTable Φ (M + 1) phiF 16 sw)).eval
            (InnerOuter.hypercubePoint (M + 1) 1 ss.challenges y) := by
    intro y
    rw [hwv y, InnerOuter.wTableMleEval_eq, InnerOuter.cMultilinearExtension_eval]
  have hfoldw : ∀ (x : F) (z : Fin (M + 1 - 1 - 1) → Fin 2),
      fold (tableFn (m := (M + 1 - 1 - 1) + 1) w_tab) x (finFunctionFinEquiv z)
        = InnerOuter.wTableMleEval Φ (M + 1) phiF 16 sw
            (InnerOuter.hypercubePoint (M + 1) (1 + 1) (Fin.snoc ss.challenges x) z) := by
    intro x z
    have hstep := fold_tableFn_eq_mle (k := M + 1 - 1 - 1) hi rfl w_tab
      (InnerOuter.wTable Φ (M + 1) phiF 16 sw) ss.challenges x hwv2 z
    rw [show (fun j : Fin (M + 1 - 1 - 1) => z (Fin.cast rfl j)) = z from rfl] at hstep
    rw [hstep, InnerOuter.wTableMleEval_eq, InnerOuter.cMultilinearExtension_eval]
  have hgv : ∀ y : Fin (M + 1 - 1) → Fin 2,
      tensorRead low high (2 ^ k)
          ((finFunctionFinEquiv y : Fin (2 ^ (M + 1 - 1))) : ℕ)
        = (InnerOuter.cMultilinearExtension (M + 1)
            (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16 ss.zc.rlin ss.zc.α ss.zc.τα)).eval
            (InnerOuter.hypercubePoint (M + 1) 1 ss.challenges y) := by
    intro y
    rw [← hav y]
    simp only [tensorRead, reidx, tensorTable, tableFn_apply]
  have hfolda : ∀ (x : F) (z : Fin (M + 1 - 1 - 1) → Fin 2),
      fold (reidx hjk' (tensorTable (tableFn (m := k) low) (tableFn (m := j) high)))
          x (finFunctionFinEquiv z)
        = (InnerOuter.cMultilinearExtension (M + 1)
            (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16 ss.zc.rlin ss.zc.α ss.zc.τα)).eval
            (InnerOuter.hypercubePoint (M + 1) (1 + 1) (Fin.snoc ss.challenges x) z) := by
    intro x z
    have hstep := fold_natTable_eq_mle (M := M) (i := 1) (k := M + 1 - 1 - 1) hi rfl
      (fun t => tensorRead low high (2 ^ k) t)
      (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16 ss.zc.rlin ss.zc.α ss.zc.τα)
      ss.challenges x hgv z
    rw [show (fun j : Fin (M + 1 - 1 - 1) => z (Fin.cast rfl j)) = z from rfl] at hstep
    rw [show (reidx hjk' (tensorTable (tableFn (m := k) low) (tableFn (m := j) high)))
        = (fun y : Fin (2 ^ ((M + 1 - 1 - 1) + 1)) =>
            tensorRead low high (2 ^ k) (y : ℕ)) from
      funext fun y => (tensorRead_eq_reidx hjk' low high y).symm]
    exact hstep
  have hre : ∀ f : Fin (2 ^ (M + 1 - 1 - 1)) → F,
      ∑ Y : Fin (2 ^ (M + 1 - 1 - 1)), f Y
        = ∑ z : Fin (M + 1 - 1 - 1) → Fin 2, f (finFunctionFinEquiv z) :=
    fun f => (Fintype.sum_equiv finFunctionFinEquiv _ f (fun _ => rfl)).symm
  have htau : ∀ (j : ℕ) (h : j < M + 1),
      ss.zc.τ₀ ⟨j, h⟩ = toExt (stmt.zc.tau0.val.getD j cpoly.field.Ext4.ZERO) := by
    intro j h
    rw [← h0v]
    rfl
  have hev : toExt e = ss.zc.τ₀ ⟨1, hi⟩ := by
    rw [htau 1 hi, he, List.getD_eq_getElem _ _ (by omega)]
  have hpref : toExt pref
      = eqProd (fun j : Fin 1 => ss.zc.τ₀ (Fin.castLE (by omega) j)) ss.challenges := by
    rw [hprefv, hc, h0v]
  have hsuf : ∀ z : Fin (M + 1 - 1 - 1) → Fin 2,
      tableFn (m := M + 1 - 1 - 1) suffix (finFunctionFinEquiv z)
        = ∏ j : Fin (M + 1 - (1 + 1)),
            (if z j = 1 then ss.zc.τ₀ ⟨1 + 1 + j.val, by have := j.isLt; omega⟩
              else 1 - ss.zc.τ₀ ⟨1 + 1 + j.val, by have := j.isLt; omega⟩) := by
    intro z
    rw [tableFn, hsufv, lagrangeBasis_get_cube]
    refine Finset.prod_congr rfl fun j _ => ?_
    rw [Vector.get_ofFn, htau (1 + 1 + j.val) (by have := j.isLt; omega)]
    rfl
  refine ⟨hgzred, hgared, ?_, ?_⟩
  · have hgzlen : gz.val.length ≤ 35 := by
      have h1 : (toRaw gz).size = (CPolynomial.Raw.smul (toExt pref) (toRaw wf)).size := by
    ...
    refine cpoly_eq_of_eval_eq (d := 34) (by norm_num) _ _
      (toUni_mem_degreeLE gz 34 (by omega))
      (degreeLE_mono' (by simp [InnerOuter.roundDegZero])
        (InnerOuter.honestComputeG Φ m₁ 16 (by norm_num) phiF 1 hi ss sw).1.2) ?_
    intro x
    rw [toUni_eval, hgzraw, raw_eval_smul, hwfraw, raw_eval_mul, ← toUni_eval inner,
      ← toUni_eval free, hinval x, hfreeval x]
    show _ = CPolynomial.eval x
      (InnerOuter.computableRoundPoly
        (InnerOuter.sumcheckPolyZero Φ (M + 1) phiF 16 ss.zc.τ₀ sw) ⟨1, hi⟩ ss.challenges)
    have hHS : InnerOuter.hypercubeSum (M + 1)
        (InnerOuter.sumcheckPolyZero Φ (M + 1) phiF 16 ss.zc.τ₀ sw) (1 + 1)
        (Fin.snoc ss.challenges x)
        = ∑ z : Fin (M + 1 - 1 - 1) → Fin 2,
            (InnerOuter.sumcheckPolyZero Φ (M + 1) phiF 16 ss.zc.τ₀ sw).eval
              (InnerOuter.hypercubePoint (M + 1) (1 + 1) (Fin.snoc ss.challenges x) z) := rfl
    rw [InnerOuter.computableRoundPoly_eval, hHS]
    have hterm : ∀ z : Fin (M + 1 - 1 - 1) → Fin 2,
        (InnerOuter.sumcheckPolyZero Φ (M + 1) phiF 16 ss.zc.τ₀ sw).eval
            (InnerOuter.hypercubePoint (M + 1) (1 + 1) (Fin.snoc ss.challenges x) z)
          = toExt pref * (tableFn (m := M + 1 - 1 - 1) suffix (finFunctionFinEquiv z)
              * InnerOuter.rangeProduct 16
                (fold (tableFn (m := (M + 1 - 1 - 1) + 1) w_tab) x (finFunctionFinEquiv z)))
            * ((1 - toExt e) * (1 - x) + toExt e * x) := by
      intro z
  ...
  · refine cpoly_eq_of_eval_eq (d := 2) (by norm_num) _ _
      (toUni_mem_degreeLE ga 2 (by omega))
      (InnerOuter.honestComputeG Φ m₁ 16 (by norm_num) phiF 1 hi ss sw).2.2 ?_
    intro x
    rw [hgaval x]
    show linSumAlpha _ _ x = CPolynomial.eval x
      (InnerOuter.computableRoundPoly
...
set_option maxHeartbeats 2000000 in
/-- The round loop of `honest_round_messages`, from any round `j` at which the
invariant holds: the statement is `honestRounds` at `j`, the witness table and
the `Ã` pair tabulate their multilinear extensions at `j`'s hypercube points,
and the `j` messages so far are the honest ones. It is the loop half of
`honest_round_messages_spec`'s proof, split out so that the peeled rounds (round
`0`, and round `1` since card T46b) can enter it at `j = 1` or `j = 2`. -/
theorem honest_round_messages_loop_spec {n μ M m₁ dRows : ℕ} (stmt : sumcheck.RoundStatement)
    (challenges : alloc.vec.Vec cpoly.field.Ext4)
    (ss : InnerOuter.NestedRoundStatement Φ (PolyVec (Rq Φ) dRows) F n μ (M + 1) m₁ 0)
    (sw : InnerOuter.LiftedWitness Φ μ n)
    (hc : WfPoint (M + 1) challenges)
    (hm0len : (alloc.vec.Vec.len stmt.zc.tau0).val = M + 1)
    (lw hg : alloc.vec.Vec cpoly.field.Ext4) (cur : sumcheck.RoundStatement)
    (out : alloc.vec.Vec sumcheck.RoundMsg) (wt : alloc.vec.Vec cpoly.field.Ext4)
    (i : Std.Usize)
    (hinv : ∃ (j : ℕ) (hj : j ≤ M + 1), i.val = j ∧
      RepRoundStmt (n := n) (μ := μ) (m₀ := M + 1) (m₁ := m₁) (i := j)
          (dRows := dRows) cur (honestRounds ss sw (toPoint (m := M + 1) challenges) j hj) ∧
        WfEvals (M + 1 - j) wt ∧
        (∀ y : Fin (M + 1 - j) → Fin 2,
          tableFn (m := M + 1 - j) wt (finFunctionFinEquiv y) =
            InnerOuter.wTableMleEval Φ (M + 1) phiF 16 sw
              (InnerOuter.hypercubePoint (M + 1) j
                (honestRounds ss sw (toPoint (m := M + 1) challenges) j hj).challenges y)) ∧
        (∃ (jj kk : ℕ) (hjk : jj + kk = M + 1 - j),
          WfEvals kk lw ∧ WfEvals jj hg ∧
          ∀ y : Fin (M + 1 - j) → Fin 2,
            reidx hjk (tensorTable (tableFn (m := kk) lw) (tableFn (m := jj) hg))
                (finFunctionFinEquiv y)
              = (InnerOuter.cMultilinearExtension (M + 1)
                  (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16
        ...
        ∀ (k : ℕ) (hk : k < j),
          RepRoundMsg
            ...
            (InnerOuter.honestComputeG Φ m₁ 16 (by norm_num) phiF k (by omega)
              (honestRounds ss sw (toPoint (m := M + 1) challenges) k (by omega)) sw)) :
    sumcheck.honest_round_messages_loop challenges (alloc.vec.Vec.len stmt.zc.tau0)
        lw hg cur out wt i
  ...
  obtain ⟨hclen, hcred⟩ := hc
  set cs := toPoint (m := M + 1) challenges with hcs
  have hMmax : M + 1 ≤ Usize.max := by rw [← hm0len]; scalar_tac
...
**Round 1 is peeled too** (card T46b): when `m₀ > 1`, its zero side comes from
`honest_compute_g_fold1_split` off the round-0 base table `w_fp` (still live),
and the four steps of a loop iteration run once outside the loop; the loop
(`honest_round_messages_loop_spec`) then starts at `j = 2`, or at `j = 1` when
`m₀ = 1`. -/
theorem honest_round_messages_spec {n μ M m₁ dRows : ℕ} (stmt : sumcheck.RoundStatement)
  ...
  step with eval_mle_layer_base_spec (k := M) w_fp a0 hWfp hRa0 as ⟨wt1, hWw1, hwfold1⟩
  have hwv0' : ∀ y : Fin (M + 1) → Fin 2,
      phiF (tableFnFp (m := M + 1) w_fp (finFunctionFinEquiv y))
        = (InnerOuter.cMultilinearExtension (M + 1)
            (InnerOuter.wTable Φ (M + 1) phiF 16 sw)).eval
            (InnerOuter.hypercubePoint (M + 1) 0 ss.challenges y) := by
  ...
  have hwv1 : ∀ y : Fin (M + 1 - (0 + 1)) → Fin 2,
      tableFn (m := M + 1 - (0 + 1)) wt1 (finFunctionFinEquiv y)
        = (InnerOuter.cMultilinearExtension (M + 1)
            (InnerOuter.wTable Φ (M + 1) phiF 16 sw)).eval
            (InnerOuter.hypercubePoint (M + 1) (0 + 1) (Fin.snoc ss.challenges (toExt a0)) y) := by
    intro y
    have hstep := fold_tableFnFp_eq_mle (M := M) (i := 0) (k := M) hzero rfl w_fp
      (InnerOuter.wTable Φ (M + 1) phiF 16 sw) ss.challenges (toExt a0) hwv0' y
    rw [show (fun j : Fin M => y (Fin.cast rfl j)) = y from rfl] at hstep
    rw [← hstep]
    exact hwfold1 (finFunctionFinEquiv y)
  ...
  have houtrep1 : ∀ (k : ℕ) (hk : k < 1),
    ...
    rw [hg]
    exact hg0rep
  have hrep1' : RepRoundStmt (n := n) (μ := μ) (m₀ := M + 1) (m₁ := m₁) (i := 1)
      (dRows := dRows) cur1 (honestRounds ss sw cs 1 (by omega)) := by
    rw [hnext1]; exact hrep1
  have hwv1' : ∀ y : Fin (M + 1 - 1) → Fin 2,
      tableFn (m := M + 1 - 1) wt1 (finFunctionFinEquiv y) =
        InnerOuter.wTableMleEval Φ (M + 1) phiF 16 sw
          (InnerOuter.hypercubePoint (M + 1) 1 (honestRounds ss sw cs 1 (by omega)).challenges y) :=
    fun y => by
      rw [hwv1 y, hnext1, InnerOuter.roundOut,
        InnerOuter.wTableMleEval_eq, InnerOuter.cMultilinearExtension_eval]
  have hjk1' : (M + 1 - min (M + 1) 10) + k0' = M + 1 - 1 := by omega
  have hav1' : ∀ y : Fin (M + 1 - 1) → Fin 2,
      reidx hjk1' (tensorTable (tableFn (m := k0') low1)
          (tableFn (m := M + 1 - min (M + 1) 10) high1)) (finFunctionFinEquiv y)
        = (InnerOuter.cMultilinearExtension (M + 1)
            (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16 ss.zc.rlin ss.zc.α ss.zc.τα)).eval
            (InnerOuter.hypercubePoint (M + 1) 1 (honestRounds ss sw cs 1 (by omega)).challenges y) :=
    fun y => by rw [hav1 y, hnext1, InnerOuter.roundOut]
  have hWhigh1 : WfEvals (M + 1 - min (M + 1) 10) high1 := by rw [hhigh1eq]; exact hWhigh0
  by_cases h1 : (1#usize : Std.Usize) < alloc.vec.Vec.len stmt.zc.tau0
  · -- round 1, peeled (card T46b): its zero side off the round-0 base table
    rw [if_pos h1]
    have hM1 : 1 < M + 1 := by rw [← hm0len]; scalar_tac
    obtain ⟨k1', hk1'⟩ : ∃ t, k0' = t + 1 := ⟨k0' - 1, by omega⟩
    have hzc1 : (honestRounds ss sw cs 1 (by omega)).zc = ss.zc := honestRounds_zc ss sw cs 1 (by omega)
    have hav1'' : ∀ y : Fin (M + 1 - 1) → Fin 2,
        reidx hjk1' (tensorTable (tableFn (m := k0') low1)
            (tableFn (m := M + 1 - min (M + 1) 10) high1)) (finFunctionFinEquiv y)
          = (InnerOuter.cMultilinearExtension (M + 1)
              (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16
                (honestRounds ss sw cs 1 (by omega)).zc.rlin
                (honestRounds ss sw cs 1 (by omega)).zc.α
                (honestRounds ss sw cs 1 (by omega)).zc.τα)).eval
              (InnerOuter.hypercubePoint (M + 1) 1
                (honestRounds ss sw cs 1 (by omega)).challenges y) := by
      rw [hzc1]; exact hav1'
    step with honest_compute_g_fold1_split_spec (n := n) (μ := μ) (M := M) (m₁ := m₁)
      (dRows := dRows) (j := M + 1 - min (M + 1) 10) (k := k0')
      cur1 w_fp a0 wt1 low1 high1 (honestRounds ss sw cs 1 (by omega)) sw hrep1' hM1 hjk1'
      hWw1 hWfp hRa0 hwfold1 hWlow1 hWhigh1 hwv1' hav1'' as ⟨g1, hg1rep⟩
    have hich1 : 1 < challenges.val.length := by omega
    step as ⟨a1, ha1v⟩
    have hRa1 : Reduced a1 := ha1v ▸ hcred _ (List.getElem_mem hich1)
    have ha1val : toExt a1 = cs ⟨1, hM1⟩ := by
      rw [hcs]
      simp only [toPoint]
      rw [ha1v, List.getD_eq_getElem _ _ hich1]
    step with round_out_spec (n := n) (μ := μ) (m₀ := M + 1) (m₁ := m₁) (i := 1)
      (dRows := dRows) cur1 g1 a1 (honestRounds ss sw cs 1 (by omega))
      (InnerOuter.honestComputeG Φ m₁ 16 (by norm_num) phiF 1 hM1
        (honestRounds ss sw cs 1 (by omega)) sw) hrep1' hg1rep hRa1 (by omega) as ⟨cur2, hrep2⟩
    step with eval_mle_layer_table_spec (M := M) (i := 1) hM1 wt1
      (InnerOuter.wTable Φ (M + 1) phiF 16 sw)
      (honestRounds ss sw cs 1 (by omega)).challenges a1 hWw1 hRa1
      (by
        intro y
        rw [hwv1' y, InnerOuter.wTableMleEval_eq, InnerOuter.cMultilinearExtension_eval])
        as ⟨wt2, hWw2, hwv2⟩
    step with alpha_split_fold_spec (j := M + 1 - min (M + 1) 10) (k := k0')
      low1 high1 a1 hWlow1 hWhigh1 hRa1 (by omega) as ⟨p2, hfA1, hfB1⟩
    obtain ⟨low2, high2⟩ := p2
    obtain ⟨hWlow2, hhigh2eq, hlow2v, htens1⟩ := hfA1 k1' hk1'
    dsimp only at hWlow2 hhigh2eq htens1
    have hjk2 : (M + 1 - min (M + 1) 10) + k1' = M + 1 - (1 + 1) := by omega
    have hfold1 : ∀ t : ℕ, t < 2 ^ ((M + 1 - min (M + 1) 10) + k1') →
        tensorRead low2 high2 (2 ^ k1') t
          = (1 - toExt a1) * tensorRead low1 high1 (2 ^ k0') (2 * t)
            + toExt a1 * tensorRead low1 high1 (2 ^ k0') (2 * t + 1) := by
      intro t ht
      have h := htens1 ⟨t, ht⟩
      rw [hk1']
      simpa only [tensorTable, tableFn_apply, tensorRead, fold, lo, ZeroCheck.hi] using h
    have hav2 := tensor_invariant_step
      (InnerOuter.alphaPublicEvals Φ (M + 1) m₁ phiF 16 ss.zc.rlin ss.zc.α ss.zc.τα)
      1 hM1 (M + 1 - min (M + 1) 10) k0' (M + 1 - min (M + 1) 10) k1'
      hjk1' hjk2 low1 high1 low2 high2 (toExt a1)
      (honestRounds ss sw cs 1 (by omega)).challenges hav1' hfold1
    have hpush1 : out1.val.length < Usize.max := by rw [houtlen1]; omega
    step as ⟨out2, hout2⟩
    have hnext2 : honestRounds ss sw cs 2 (by omega)
        = InnerOuter.roundOut Φ (M + 1) m₁ 16 (honestRounds ss sw cs 1 (by omega))
            (InnerOuter.honestComputeG Φ m₁ 16 (by norm_num) phiF 1 hM1
              (honestRounds ss sw cs 1 (by omega)) sw) (cs ⟨1, hM1⟩) := rfl
    rw [ha1val] at hrep2 hwv2 hav2
    have houtlen2 : out2.val.length = 2 := by
      rw [hout2, List.length_append, houtlen1]; simp
    have houtrep2 : ∀ (k : ℕ) (hk : k < 2),
        RepRoundMsg
          (out2.val.getD k { g_zero := alloc.vec.Vec.new cpoly.field.Ext4,
                             g_alpha := alloc.vec.Vec.new cpoly.field.Ext4 })
          (InnerOuter.honestComputeG Φ m₁ 16 (by norm_num) phiF k (by omega)
            (honestRounds ss sw cs k (by omega)) sw) := by
      intro k hk
      rcases Nat.lt_or_ge k 1 with hklt | hkge
      · rw [hout2, getD_append_lt _ _ _ (by omega)]
        exact houtrep1 k hklt
      · have hkeq : k = 1 := by omega
        subst hkeq
        have hgetd : out2.val.getD 1
            { g_zero := alloc.vec.Vec.new cpoly.field.Ext4,
              g_alpha := alloc.vec.Vec.new cpoly.field.Ext4 } = g1 := by
          rw [hout2, show (1 : ℕ) = out1.val.length from houtlen1.symm, getD_append_eq]
        rw [hgetd]
        exact hg1rep
    apply honest_round_messages_loop_spec stmt challenges ss sw ⟨hclen, hcred⟩ hm0len
    exact ⟨2, by omega, by scalar_tac, by rw [hnext2]; exact hrep2, hWw2,
      (fun y => by
        rw [hwv2 y, hnext2, InnerOuter.roundOut,
          InnerOuter.wTableMleEval_eq, InnerOuter.cMultilinearExtension_eval]),
      ⟨M + 1 - min (M + 1) 10, k1', by omega, hWlow2, by rw [hhigh2eq]; exact hWhigh1,
        (fun y => by rw [hav2 y, hnext2, InnerOuter.roundOut])⟩,
      houtlen2, houtrep2⟩
  · -- `m₀ = 1`: no round 1, the loop runs from `j = 1` (and at once exits)
    rw [if_neg h1]
    apply honest_round_messages_loop_spec stmt challenges ss sw ⟨hclen, hcred⟩ hm0len
    exact ⟨1, by omega, by scalar_tac, hrep1', hWw1, hwv1',
      ⟨M + 1 - min (M + 1) 10, k0', hjk1', hWlow1, hWhigh1, hav1'⟩,
      houtlen1, houtrep1⟩
