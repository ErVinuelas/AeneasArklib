...
Those carry `sorry` and are this file's proof debt; their statements are
known-true, having been proved upstream at the other version, which is the best
kind of obligation to hand a prover.

The file therefore lives in `lean-wip/` until those are discharged.
...
theorem toExt_eq_iff (a b : cpoly.field.Ext4) :
  ...
  -- PORTED, NOT RE-PROVEN. The original proof of this exact statement is
  -- inlined below verbatim from AeneasCompPoly `583cfaff0617`,
  -- `cpoly/lean/Field.lean`. It walks a monadic body identical to this
  -- project's, and fails here only on the version gap in the file header
  -- (Lean v4.32.0 / CompPoly `c0fcf450` / Mathlib `81a5d257` there, against
  -- v4.33.1 / `a09455a2` / `0df444a3` here). **Adapt it; do not reinvent it.**
  --
  -- ORIGINAL:
  --   constructor
  --   · intro h
